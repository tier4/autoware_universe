...
{
  const double measurement_yaw = autoware::universe_utils::normalizeRadian(tf2::getYaw(measurement_quat));
  ...
  const types::DynamicObject & measurement_object, const std::uint8_t measurement_label) const
{
  ...
  const double dist =
    autoware::universe_utils::calcDistance2d(measurement_object.pose.position, tracked_object.pose.position);
