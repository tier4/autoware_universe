  ...
  std::unordered_map<lanelet::Id, uint8_t> & traffic_signal_overrides)
{
  for (const std::string & intersection_id : target_intersection_ids) {
    ...
    const std::vector<lanelet::Id> & reg_elem_ids_in_intersection =
      signals_by_intersection_.at(intersection_id);

      
    const auto active_signal_heads = buildObservedSignalHeads(reg_elem_ids_in_intersection, traffic_light_id_map);
    for (const std::vector<lanelet::Id> & signal_head_member_ids : active_signal_heads) {
      const SignalHeadPhase phase =
        classifySignalHeadPhase(signal_head_member_ids, traffic_light_id_map);

      for (const lanelet::Id reg_elem_id : signal_head_member_ids) {
        ...
        const auto & traffic_light = *reg_elem_it;
        ...
        for (const std::pair<const std::string, lanelet::Attribute> & attribute :
          ...
          overwrite_to_color_by_phase(
            phase, color_mapping->first, color_mapping->second,
            parse_ids(attribute.second.value()), traffic_signal_overrides);
        }
      }
    }
  }
...
void CrosswalkTrafficLightEstimatorNode::overwrite_to_color_by_phase(
  ...
  std::unordered_map<lanelet::Id, uint8_t> & traffic_signal_overrides) const
{
  const auto overwrite_targets = [&](const uint8_t color) {
    for (const lanelet::Id target_reg_elem_id : target_reg_elem_ids) {
      traffic_signal_overrides[target_reg_elem_id] = color;
    }
  };
...
std::vector<std::vector<lanelet::Id>> CrosswalkTrafficLightEstimatorNode::buildObservedSignalHeads(
  const std::vector<lanelet::Id> & traffic_light_reg_elem_ids,
  const TrafficLightIdMap & traffic_light_id_map) const
{
  // A signal head bundles reg_elems whose physical faces match exactly; matching by exact set
  // (not overlap) keeps independent phases such as green arrows apart.
  ...
  // Keep only heads with at least one observed member; an unobserved head (opposing/cross facing
  // away) carries no "from" color and yields no override. Members within a kept head are not
  // filtered, so color aggregation stays robust to a single occluded member.
  std::vector<std::vector<lanelet::Id>> observed_signal_heads;
  for (auto & [face_linestring_ids, same_head_reg_elem_ids] : reg_elem_ids_by_face_set) {
    if (isSignalHeadObserved(same_head_reg_elem_ids, traffic_light_id_map)) {
      observed_signal_heads.push_back(std::move(same_head_reg_elem_ids));
    }
  }
  return observed_signal_heads;
}

bool CrosswalkTrafficLightEstimatorNode::isSignalHeadObserved(
  const std::vector<lanelet::Id> & signal_head_member_ids,
  const TrafficLightIdMap & traffic_light_id_map) const
{
  return std::any_of(
    signal_head_member_ids.begin(), signal_head_member_ids.end(),
    [&](const lanelet::Id member_reg_elem_id) {
      return traffic_light_id_map.count(member_reg_elem_id) > 0;
    });
}
...
CrosswalkTrafficLightEstimatorNode::classifySignalHeadPhase(
  ...
  const TrafficLightIdMap & traffic_light_id_map) const
{
  const auto circle_color = getSignalHeadCircleColor(signal_head_member_ids, traffic_light_id_map);
  ...
  if (
    isSignalHeadShowsStraightArrow(signal_head_member_ids, traffic_light_id_map) ||
  ...
  if (isSignalHeadShowsGreenArrow(signal_head_member_ids, traffic_light_id_map)) {
  ...
  const TrafficLightIdMap & traffic_light_id_map) const
{
  ...
  for (const auto member_reg_elem_id : signal_head_member_ids) {
  ...
  const TrafficSignalArray::ConstSharedPtr msg)
{
  ...
  estimateIntersectionTrafficSignal(
    target_intersection_ids, traffic_light_id_map, traffic_signal_overrides);

  // Write the crosswalk (pedestrian) and intersection (vehicle) overrides into the output.
  applyOverridesToTrafficSignals(traffic_signal_overrides, output);
...
void CrosswalkTrafficLightEstimatorNode::applyOverridesToTrafficSignals(
  ...
  TrafficSignalArray & output)
{
  ...
  for (const auto & [group_id, color] : traffic_signal_overrides) {
