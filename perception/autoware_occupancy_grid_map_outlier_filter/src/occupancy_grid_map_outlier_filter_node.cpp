...
: Node("OccupancyGridMapOutlierFilter", options)
{
  ...
  pointcloud_pub_ =
    AUTOWARE_CREATE_PUBLISHER2(PointCloud2, "~/output/pointcloud", rclcpp::SensorDataQoS());
  ...
  const OccupancyGrid::ConstSharedPtr & input_ogm, const PointCloud2::ConstSharedPtr & input_pc)
{
  ...
  auto base_link_frame_filtered_pc_ptr = ALLOCATE_OUTPUT_MESSAGE_UNIQUE(pointcloud_pub_);
