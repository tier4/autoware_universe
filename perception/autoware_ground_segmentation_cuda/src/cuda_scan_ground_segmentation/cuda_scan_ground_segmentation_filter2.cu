...
/**
 * @brief CUDA kernel to initialize an array of ClassifiedPointType elements.
 *
 ...
 *
 * @param arr Pointer to the array of ClassifiedPointType to initialize.
 * @param N The number of points in the array to initialize.
 */
__global__ void initPoints(ClassifiedPointType * arr, uint32_t N)
{
...
__global__ void getCellNumPointsKernel(
  const Cell * __restrict__ centroid_cells_list_dev, const uint32_t num_cells,
...
__global__ void assignPointToClassifyPointKernel(
  cuda::PointCloud2::Ptr input_points, const uint32_t num_points,
  const Cell * __restrict__ cells_centroid_dev, uint32_t * __restrict__ cell_counts_dev,
  FilterParameters filter_parameters_dev,
  ClassifiedPointType * __restrict__ classified_points_dev)
{
  ...
  auto p = input_points[idx];
  const float x = p.x - filter_parameters_dev.center_x;
  const float y = p.y - filter_parameters_dev.center_y;
  ...
  assign_classified_point_dev.z = p.z;
...
__device__ void updateGndPointInCell(
  Cell & cell, const ClassifiedPointType & gnd_point)
{
 ...
 *
 * @param cell Reference to the Cell structure representing the cell to update.
 * @param point The ClassifiedPointType representing the ground point to remove.
 */
__device__ void removeGndPointInCell(Cell & cell, const ClassifiedPointType & point)
{
  cell.gnd_height_avg =
...
__device__ void RecursiveGndCellSearch(
  const Cell * __restrict__ sector_cells_list_dev,
 ...
 *
 * @param sector_cells_list_dev Pointer to the array of Cell structures representing the
...
__device__ void SegmentInitializedCell(
  Cell * __restrict__ sector_cells_list_dev,
  ClassifiedPointType * __restrict__ cell_classify_points_dev,
  FilterParameters filter_parameters_dev, const uint32_t cell_idx_in_sector)
{
...
__global__ void scanPerSectorGroundReferenceKernel(
  ClassifiedPointType * __restrict__ classified_points_dev,
  Cell * __restrict__ centroid_cells_list_dev,
  FilterParameters filter_parameters_dev)
{
  ...
  Cell * sector_cells_list_dev = &centroid_cells_list_dev[sector_start_cell_index];
  for (int cell_index_in_sector = 0;
    ...
    ClassifiedPointType * cell_classify_points_dev =
...
__global__ void CellsCentroidInitializeKernel(
  Cell * __restrict__ centroid_cells_list_dev, const uint32_t max_num_cells)
{
  uint32_t idx = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void calcCellPointNumberKernel(
  cuda::PointCloud2::Ptr input_points, const uint32_t num_input_points,
  FilterParameters filter_parameters_dev,
  Cell * __restrict__ centroid_cells_list_dev)
{
  uint32_t idx = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void scatterKernel(
  cuda::PointCloud2::Ptr input_points, const uint32_t * __restrict__ flags,
  const uint32_t * __restrict__ indices, const uint32_t num_points,
  cuda::PointCloud2::Ptr output_points)
{
  uint32_t idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx >= num_points) {
    return;
...
__global__ void updateCellStartPointIndexKernel(
  Cell * __restrict__ centroid_cells_list_dev,
  const uint32_t * __restrict__ cell_first_point_indices_dev, const uint32_t max_num_cells)
{
  uint32_t idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx >= max_num_cells) {
    return;  // Out of bounds
...
: filter_parameters_(filter_parameters)
{
  ...
  input_points_dev_.reset(new cuda::PointCloud2(stream_, mempool_));
  ...
  ClassifiedPointType * classified_points_dev, Cell * centroid_cells_list_dev)
{
  ...
  scanPerSectorGroundReferenceKernel<<<grid_dim, block_dim, 0, stream_->get()>>>(
    classified_points_dev, centroid_cells_list_dev, filter_parameters_);
  CHECK_CUDA_ERROR(cudaGetLastError());
  // For debug
  CHECK_CUDA_ERROR(cudaDeviceSynchronize());
  std::cerr << "End of scanPerSectorGroundRef " << std::endl;
...
void CudaScanGroundSegmentationFilter::countCellPointNum(Cell * centroid_cells_list_dev)
{
  ...
  CellsCentroidInitializeKernel<<<cells_grid_dim, block_dim, 0, stream_->get()>>>(
    centroid_cells_list_dev, filter_parameters_.max_num_cells);
  CHECK_CUDA_ERROR(cudaGetLastError());

  calcCellPointNumberKernel<<<points_grid_dim, block_dim, 0, stream_->get()>>>(
    input_points_dev_->data(), number_input_points_, filter_parameters_, centroid_cells_list_dev);
  CHECK_CUDA_ERROR(cudaGetLastError());
}
  ...
  ClassifiedPointType * classified_points_dev)
{
  ...
  initPoints<<<grid_dim, block_dim, 0, stream_->get()>>>(
    classified_points_dev, number_input_points_);
  CHECK_CUDA_ERROR(cudaGetLastError());

  assignPointToClassifyPointKernel<<<grid_dim, block_dim, 0, stream_->get()>>>(
    input_points_dev_->data(), number_input_points_, centroid_cells_list_dev, cell_counts_dev,
    filter_parameters_, classified_points_dev);
  CHECK_CUDA_ERROR(cudaGetLastError());
  ...
  cuda::PointCloud2 & output_points_dev, const PointType pointtype)
{
  ...
  dim3 block_dim(512);
  dim3 grid_dim((number_input_points_ + block_dim.x - 1) / block_dim.x);
  setFlagsKernel<<<grid_dim, block_dim, 0, stream_->get()>>>(
    flag_dev.data(), number_input_points_, 0);
  CHECK_CUDA_ERROR(cudaGetLastError());
  ...
  device_vector<uint32_t> & cell_first_point_indices_dev)
{
  ...
  getCellNumPointsKernel<<<blocks, threads, 0, stream_->get()>>>(
    centroid_cells_list_dev.data(), filter_parameters_.max_num_cells, num_points_per_cell_dev.data());
  CHECK_CUDA_ERROR(cudaGetLastError());

  CHECK_CUDA_ERROR(cuda::ExclusiveScan(num_points_per_cell_dev, cell_first_point_indices_dev));

  // update start point index in centroid_cells_list_dev
  updateCellStartPointIndexKernel<<<blocks, threads, 0, stream_->get()>>>(
    centroid_cells_list_dev.data(), cell_first_point_indices_dev.data(), filter_parameters_.max_num_cells);
  CHECK_CUDA_ERROR(cudaGetLastError());
}
...
{
  input_points_dev_->from_point_cloud2(input_points);
  dev_output_points_->reserve(input_points_dev_->size());
  dev_ground_points_->reserve(input_points_dev_->size());

  number_input_points_ = input_points_dev_->size();

  const auto & max_num_cells = filter_parameters_.max_num_cells;
  device_vector<Cell> centroid_cells_list_dev(max_num_cells, stream_, mempool_);
  device_vector<uint32_t> num_points_per_cell_dev(max_num_cells, stream_, mempool_);
  device_vector<uint32_t> cell_first_point_indices_dev(max_num_cells, stream_, mempool_);
  device_vector<ClassifiedPointType> classified_points_dev(number_input_points_, stream_, mempool_);
  ...
  assignPointToClassifyPoint(
    centroid_cells_list_dev.data(), cell_counts_dev.data(), classified_points_dev.data());

    // // For debug
    // std::vector<ClassifiedPointType> test_vec;
    // classified_points_dev.to_vector(test_vec);

    // for (size_t i = 0; i < test_vec.size(); ++i) {
    //   printf("Point %lu is at %lu\n", i, test_vec[i].origin_index);
    // }
