...
void TrackerProcessor::mergeOverlappedTracker(const rclcpp::Time & time)
{
  ...
  for (size_t i = 0; i < valid_trackers.size(); ++i) {
    ...
    for (const auto & [p2, idx2] : nearby) {
      ...
      if (data1.label == 7 && data2.label == 7) {
        ...
        if (canMergePedestrianTargets(*data1.tracker, *data2.tracker, time, std::sqrt(dist_sq))) {
  ...
  const double distance) const
{
  ...
  if (target_known_prob >= min_known_prob) {
    ...
    // Both are known class, check distance
    if (distance < config_.min_known_object_removal_iou * 10) {  // Reusing iou threshold as
                                                                 // distance
  ...
  // The target class is unknown, check distance
  if (distance < config_.min_known_object_removal_iou * 10) {  // Reusing iou threshold as distance
    if (other_known_prob < min_known_prob) {
