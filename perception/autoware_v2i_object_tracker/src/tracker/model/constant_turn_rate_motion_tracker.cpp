...
#include "autoware_v2i_object_tracker/tracker/model/constant_turn_rate_motion_tracker.hpp"
  ...
  const geometry_msgs::msg::Transform & self_transform)
{
  ...
  if (enable_yaw_measurement) {
    ...
    const auto yaw = [&] {
      auto obj_yaw = autoware_utils::normalize_radian(
  ...
  const auto C = autoware::v2i_object_tracker::utils::stackMatricesVertically(C_list);
  const auto Y = autoware::v2i_object_tracker::utils::stackMatricesVertically(Y_list);
  const auto R = autoware::v2i_object_tracker::utils::stackMatricesDiagonally(R_block_list);
