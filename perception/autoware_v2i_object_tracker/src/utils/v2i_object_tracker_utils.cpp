...
namespace autoware::v2i_object_tracker::utils
{
  ...
  const std::string & target_frame_id, const rclcpp::Time & time)
{
  try {
    ...
    RCLCPP_WARN_STREAM(rclcpp::get_logger("v2i_object_tracker"), ex.what());
    return boost::none;
  }
}
  ...
  const double max_angle_diff_from_lane)
{
  ...
  if (velocity_is_reverted) {
    object_motion_yaw = autoware_utils::normalize_radian(object_yaw + M_PI);
  }
  ...
  const lanelet::ConstLanelets & lanelets, const double max_lateral_velocity)
{
  ...
  const double object_vel_yaw_global =
    autoware_utils::normalize_radian(object_yaw + object_vel_yaw);
