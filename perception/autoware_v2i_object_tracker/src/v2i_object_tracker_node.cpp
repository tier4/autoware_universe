...
#include "v2i_object_tracker_node.hpp"

#include "autoware_v2i_object_tracker/utils/v2i_object_tracker_utils.hpp"
#include "autoware_v2i_object_tracker/utils/utils.hpp"
...
namespace autoware::v2i_object_tracker
{
using Label = autoware_perception_msgs::msg::ObjectClassification;

V2iObjectTrackerNode::V2iObjectTrackerNode(const rclcpp::NodeOptions & node_options)
: rclcpp::Node("v2i_object_tracker", node_options),
  tf_buffer_(this->get_clock()),
  tf_listener_(tf_buffer_)
{
  // glog for debug
  if (!google::IsGoogleLoggingInitialized()) {
    google::InitGoogleLogging("v2i_object_tracker");
  ...
  detected_object_sub_ = create_subscription<autoware_perception_msgs::msg::DetectedObjects>(
    "input", rclcpp::QoS{1},
    std::bind(&V2iObjectTrackerNode::onMeasurement, this, std::placeholders::_1));
  ...
  sub_map_ = this->create_subscription<LaneletMapBin>(
    ...
    std::bind(&V2iObjectTrackerNode::onMap, this, std::placeholders::_1));
  ...
  if (tracker_config_directory_.empty()) {
    tracker_config_directory_ =
      ament_index_cpp::get_package_share_directory("v2i_object_tracker") + "/config/tracking/";
  }
  ...
  if (enable_delay_compensation) {
    ...
    publish_timer_ = rclcpp::create_timer(
      this, get_clock(), period_ns, std::bind(&V2iObjectTrackerNode::onTimer, this));
  }
...
void V2iObjectTrackerNode::onMap(const LaneletMapBin::ConstSharedPtr msg)
{
  RCLCPP_INFO(get_logger(), "[V2i Object Tracker]: Start loading lanelet");
  lanelet_map_ptr_ = std::make_shared<lanelet::LaneletMap>();
  lanelet::utils::conversion::fromBinMsg(
    *msg, lanelet_map_ptr_, &traffic_rules_ptr_, &routing_graph_ptr_);
  RCLCPP_INFO(get_logger(), "[V2i Object Tracker]: Map is loaded");
  map_is_loaded_ = true;
}

void V2iObjectTrackerNode::onMeasurement(
  const autoware_perception_msgs::msg::DetectedObjects::ConstSharedPtr input_objects_msg)
{
  const auto self_transform = autoware::v2i_object_tracker::utils::getTransformAnonymous(
...
std::shared_ptr<Tracker> V2iObjectTrackerNode::createNewTracker(
...
void V2iObjectTrackerNode::onTimer()
{
...
void V2iObjectTrackerNode::checkTrackerLifeCycle(
...
void V2iObjectTrackerNode::mapBasedNoiseFilter(
  std::list<std::shared_ptr<Tracker>> & list_tracker, const rclcpp::Time & time)
{
  for (auto itr = list_tracker.begin(); itr != list_tracker.end(); ++itr) {
    autoware_perception_msgs::msg::TrackedObject object;
    (*itr)->getTrackedObject(time, object);
    const auto closest_lanelets = autoware::v2i_object_tracker::utils::getClosestValidLanelets(
    ...
    const bool is_velocity_direction_close_to_lanelet =
      autoware::v2i_object_tracker::utils::hasValidVelocityDirectionToLanelet(
...
void V2iObjectTrackerNode::distanceBasedNoiseFilter(
  std::list<std::shared_ptr<Tracker>> & list_tracker, const rclcpp::Time & time,
...
void V2iObjectTrackerNode::sanitizeTracker(
  std::list<std::shared_ptr<Tracker>> & list_tracker, const rclcpp::Time & time)
{
...
inline bool V2iObjectTrackerNode::shouldTrackerPublish(
...
void V2iObjectTrackerNode::publish(const rclcpp::Time & time) const
{
...
RCLCPP_COMPONENTS_REGISTER_NODE(autoware::v2i_object_tracker::V2iObjectTrackerNode)
