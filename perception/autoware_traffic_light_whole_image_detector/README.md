...
ROS2 package that runs the CoMLOps large detection model (regression model similar to [trt-lightnet](https://github.com/your-org/trt-lightnet)) on whole images and publishes **traffic light detection ROIs** as `tier4_perception_msgs/msg/DetectedObjectsWithFeature`.
...
- **Output:** `tier4_perception_msgs/msg/DetectedObjectsWithFeature` (detected objects with 2D ROI feature)
...
| `traffic_light_class_name` | string | `TRAFFIC_LIGHT` | Only publish detections for this class; empty = all detections |

## Dependencies

- **TensorRT, CUDA, cuDNN** (required for real inference): Same as `autoware_traffic_light_classifier`. If not all found at build time, the package builds a **stub** that publishes empty DetectedObjectsWithFeature and logs: *"Whole image detector built without TensorRT"*.

### Enable inference (fix "publishing empty objects")
...
- **Publish:** `~/output/rois` (`tier4_perception_msgs/msg/DetectedObjectsWithFeature`)
