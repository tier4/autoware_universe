...
TEST(MultiCameraFusionFuse, MapBasedFilterDropsArrowNotDeclaredOnCircleOnlyMap)
{
  ...
  auto config = make_default_config();
  config.use_map_based_filter = true;
...
TEST(MultiCameraFusionFuse, MapBasedFilterProtectsValidPredictionFromBogusHigherConfidence)
{
  // Arrange
  ...
  TrafficLight bogus_signal;
  bogus_signal.traffic_light_id = RIGHT_TRAFFIC_LIGHT_ID;
  bogus_signal.elements.push_back(arrow_element);
  const auto input1 = make_fusion_input("camera1", bogus_signal);
