...
def overwrite_config(
    param_dict, launch_config_name, node_params_name, context
):
    if LaunchConfiguration(launch_config_name).perform(context) != "":
        param_dict[node_params_name] = LaunchConfiguration(
            launch_config_name
        ).perform(context)
...
def fusion_config_sanity_check(fusion_config: dict):
    ...
    for param_name in listed_param_names:
        ...
        if param_name not in fusion_config:
            raise Exception(
                "Error: " + param_name + " is not in the config file"
            )
...
def get_ogm_creation_config(total_config: dict, list_iter: int) -> dict:
    ...
    ogm_creation_config["scan_origin_frame"] = fusion_config_[
        "each_ogm_sensor_frames"
    ][list_iter]
...
def launch_setup(context, *args, **kwargs):
    ...
    for i in range(number_of_nodes):
        ...
        ogm_creation_config["updater_type"] = LaunchConfiguration(
            "updater_type"
        ).perform(context)
        ...
        node = ComposableNode(
            ...
            name="occupancy_grid_map_node_in_"
            + ogm_creation_config["scan_origin_frame"],
            remappings=[
                (
                    "~/input/obstacle_pointcloud",
                    fusion_config["obstacle_pointcloud_topic"],
                ),
                (
                    "~/input/raw_pointcloud",
                    fusion_config["raw_pointcloud_topics"][i],
                ),
                (
                    "~/output/occupancy_grid_map",
                    fusion_config["fusion_input_ogm_topics"][i],
                ),
            ],
    ...
    occupancy_grid_map_container = ComposableNodeContainer(
        ...
        composable_node_descriptions=gridmap_generation_composable_nodes
        + gridmap_fusion_node,
