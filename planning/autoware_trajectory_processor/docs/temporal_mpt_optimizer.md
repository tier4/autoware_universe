...
Horizon references are built directly in `TrajectoryTemporalMPTOptimizer` before each solve:
...
1. Add the plugin class to `plugin_names` in `config/trajectory_processor.param.yaml` (position sets when it runs relative to other plugins).
...
Plugin-specific parameters live in `config/plugins/trajectory_temporal_mpt_optimizer.param.yaml` (loaded from `launch/trajectory_processor.launch.xml`).
...
| `enable_debug_info`             | `true`                                | Extra `RCLCPP_INFO` / `WARN` on solve success or failure                                                             |
| `publish_debug_topics`          | `true`                                | Publish reference, odometry, output trajectory, solve status, and control debug topics under `~/debug/temporal_mpt/` |
...
| `log_replay_fixture_to_console` | `true`                                | Log fixture path and contents to the console                                                                         |
| `reroute_output`                | `true`                                | Apply MPC updates only to debug output, leaving the pipeline trajectory unchanged                                    |
...
```bash
ros2 run autoware_trajectory_processor temporal_mpt_debug_visualizer.py
```
