...
Trajectory getStopTrajectory(const Trajectory & trajectory, const int nearest_traj_idx, const double current_vel, const double decel)
{
  ...
  for (size_t i = nearest_traj_idx+1; i < trajectory.points.size(); ++i) {
    ...
    const float interpolated_velocity = current_vel * (stopping_distance - accumulated_distance) / stopping_distance;
