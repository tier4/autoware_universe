  ...
  const std::vector<rclcpp::Parameter> & received_parameters)
{
  ...
  // obstacle_stop.obstacle_filtering
  {
    ...
    upd_p(pc_ns + "pointcloud_cluster_tolerance", pc_filter_p.pointcloud_cluster_tolerance);
    upd_p(pc_ns + "pointcloud_min_cluster_size", pc_filter_p.pointcloud_min_cluster_size);
    upd_p(pc_ns + "pointcloud_max_cluster_size", pc_filter_p.pointcloud_max_cluster_size);
    ...
    auto update_target_type_vector = [&](const std::string & type_group_prefix) {
      ...
      for (const auto & type : types_map) {  // またはキャプチャした object_types_map_const
        const std::string param_name_to_check = type_group_prefix + type.first;
        bool value_from_param_update_call{false};
        upd_p(param_name_to_check, value_from_param_update_call);
        if (value_from_param_update_call) {
          ret_vector.push_back(type.second);
        }
      }
      std::sort(ret_vector.begin(), ret_vector.end());
      return ret_vector;
    };

    p.inside_stop_object_types = update_target_type_vector(of_ns + "object_type.inside.");
    p.outside_stop_object_types = update_target_type_vector(of_ns + "object_type.outside.");
  ...
  const double dist_to_bumper)
{
  ...
  {
    autoware_utils::ScopedTimeTrack st_clustering_process("clustering_process", *time_keeper_);
  ...
  const TrajectoryPolygonCollisionCheck & trajectory_polygon_collision_check)
{
  ...
  const auto cropped_decimated_traj_points = [&]() {
    ...
    const double crop_length =
      (stop_planning_param_.stop_margin + vel * vel * 0.5 / -common_param_.min_accel) * scale;
