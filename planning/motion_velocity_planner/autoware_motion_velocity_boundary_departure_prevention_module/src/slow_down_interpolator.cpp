  ...
  const SideKey side_key) const
{
  ...
  const auto rel_dist_m = std::clamp(
    (interp_vel_mps * interp_vel_mps - curr_vel * curr_vel) / (2 * exp_decel_mps2), 0.0,
    lon_dist_to_bound_m);

  fmt::print(
    "lat_dist_to_bound_m: {:.3f}, interp_vel_mps: {:.3f}, exp_vel: {:.3f}\n", lat_dist_to_bound_m,
    interp_vel_mps, exp_vel);
  ...
  const double curr_vel, const double lat_dist, const SideKey side_key) const
{
  ...
  const auto [min_vel, max_vel] = get_vel_axis();
  ...
  const auto slope = (lat_dist - min_dist) / (max_dist - min_dist);
  const auto ratio = std::clamp(slope, 0.0, 1.0);
  return std::min(min_vel + ratio * (max_vel - min_vel), curr_vel);
}
...
std::array<double, 2> SlowDownInterpolator::get_vel_axis() const
{
