  ...
  const bool is_departure_persist)
{
  ...
  while (idx < departure_points.size()) {
    ...
    while (idx_end < departure_points.size()) {
      ...
      if (curr.departure_type == DepartureType::CRITICAL_DEPARTURE) {
        break;
      }
  ...
  const double th_pt_shift_dist_m, const double th_pt_shift_angle_rad, const double dist_to_stop)
{
  for (auto & crit_dpt_pt_mut : critical_departure_points) {
    ...
    if (crit_dpt_pt_mut.dist_on_traj > (offset_from_ego + dist_to_stop)) {
      crit_dpt_pt_mut.can_be_removed = true;
      continue;
    }
    ...
    for (const auto & dpt_pt : new_departure_points[side_key]) {
      ...
      const auto is_near_curr_pts = std::any_of(
        ...
        [&](const CriticalDeparturePoint & crit_pt) {
          return std::abs(dpt_pt.dist_on_traj - crit_pt.dist_on_traj) < th_new_point_min_distance_m;
        });
...
std::vector<std::tuple<Pose, Pose, double>> get_slow_down_intervals(
  ...
  [[maybe_unused]] const VehicleInfo & vehicle_info, const double curr_vel,
  [[maybe_unused]] const TriggerThreshold & th_trigger, const double ego_dist_on_traj_m)
{
  ...
  for (auto && [idx, departure_interval] : departure_intervals | ranges::views::enumerate) {
    const auto [slow_down_pt_on_traj, slow_down_dist_on_traj_m] = std::make_pair(
    ...
    departure_interval.prev_vel =
      autoware::signal_processing::lowpassFilter(vel, departure_interval.prev_vel, 0.99);

    auto target_pose = ref_traj_pts.compute(ego_dist_on_traj_m + rel_dist_m);

    slowdown_intervals.emplace_back(target_pose.pose, end_pose, departure_interval.prev_vel);
  }
...
{
  constexpr auto target_vel{0.0};
  return std::abs((th_vel * th_vel - target_vel * target_vel) / (2.0 * decel));
};
...
{
  std::unordered_map<DepartureType, size_t> cutoff_time_idx;

  for (size_t i = 0; i < ego_pred_path.size(); ++i) {
    const auto & pt_time = rclcpp::Duration(ego_pred_path[i].time_from_start).seconds();
    if (
      pt_time >= th_cutoff_time_departure_s &&
      cutoff_time_idx.find(DepartureType::CRITICAL_DEPARTURE) == cutoff_time_idx.end()) {
      cutoff_time_idx[DepartureType::CRITICAL_DEPARTURE] = i;
    }

    if (
      pt_time >= th_cutoff_time_near_boundary_s &&
      cutoff_time_idx.find(DepartureType::NEAR_BOUNDARY) == cutoff_time_idx.end()) {
      cutoff_time_idx[DepartureType::NEAR_BOUNDARY] = i;
    }

    if (
      cutoff_time_idx.find(DepartureType::CRITICAL_DEPARTURE) != cutoff_time_idx.end() &&
      cutoff_time_idx.find(DepartureType::NEAR_BOUNDARY) != cutoff_time_idx.end()) {
      return cutoff_time_idx;
    }
  }

  if (cutoff_time_idx.find(DepartureType::CRITICAL_DEPARTURE) == cutoff_time_idx.end()) {
    cutoff_time_idx[DepartureType::CRITICAL_DEPARTURE] = ego_pred_path.size() - 1;
  }

  if (cutoff_time_idx.find(DepartureType::NEAR_BOUNDARY) == cutoff_time_idx.end()) {
    cutoff_time_idx[DepartureType::NEAR_BOUNDARY] = ego_pred_path.size() - 1;
  }
