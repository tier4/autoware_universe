...
void RunOutModule::init(rclcpp::Node & node, const std::string & module_name)
{
  ...
  timekeeper_publisher_ =
    node.create_publisher<autoware_utils::ProcessingTimeDetail>("~/" + ns_ + "processing_time", 1);
  time_keeper_ = std::make_shared<autoware_utils::TimeKeeper>(timekeeper_publisher_, &std::cerr);
