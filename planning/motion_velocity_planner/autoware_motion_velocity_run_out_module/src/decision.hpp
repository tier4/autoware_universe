...
{
  // TODO(Maxime): find good conditions to slowdown
  return false;
}
...
inline DecisionType calculate_decision_type(
  ...
  std::stringstream & explanation, const bool ego_is_stopped, const Parameters & params)
{
  if (condition_to_stop(history, collision_type, now, explanation, ego_is_stopped, params)) {
    return stop;
  }
  if (condition_to_slowdown()) {
    return slowdown;
  }
  return nothing;
}
...
inline void calculate_decisions(
  ...
  const rclcpp::Time & now, const bool ego_is_stopped, const Parameters & params)
{
    ...
    for (const auto & collision : object.collisions) {
      Decision d(
        collision, calculate_decision_type(
                     collision.type, decision_history, now, explanation, ego_is_stopped, params));
