...
: CruisePlannerInterface(node, common_param, cruise_planning_param)
{
  ...
  dense_resampling_time_interval_ = node.declare_parameter<double>(
    "obstacle_cruise.cruise_planning.optimization_based_planner.dense_resampling_time_interval");
  sparse_resampling_time_interval_ = node.declare_parameter<double>(
    "obstacle_cruise.cruise_planning.optimization_based_planner.sparse_resampling_time_interval");
  dense_time_horizon_ = node.declare_parameter<double>(
    "obstacle_cruise.cruise_planning.optimization_based_planner.dense_time_horizon");
  max_time_horizon_ = node.declare_parameter<double>(
    "obstacle_cruise.cruise_planning.optimization_based_planner.max_time_horizon");

  t_dangerous_ = node.declare_parameter<double>(
    "obstacle_cruise.cruise_planning.optimization_based_planner.t_dangerous");
  velocity_margin_ = node.declare_parameter<double>(
    "obstacle_cruise.cruise_planning.optimization_based_planner.velocity_margin");

  replan_vel_deviation_ = node.declare_parameter<double>(
    "obstacle_cruise.cruise_planning.optimization_based_planner.replan_vel_deviation");
  engage_velocity_ = node.declare_parameter<double>(
    "obstacle_cruise.cruise_planning.optimization_based_planner.engage_velocity");
  engage_acceleration_ = node.declare_parameter<double>(
    "obstacle_cruise.cruise_planning.optimization_based_planner.engage_acceleration");
  engage_exit_ratio_ = node.declare_parameter<double>(
    "obstacle_cruise.cruise_planning.optimization_based_planner.engage_exit_ratio");
  stop_dist_to_prohibit_engage_ = node.declare_parameter<double>(
    "obstacle_cruise.cruise_planning.optimization_based_planner.stop_dist_to_prohibit_engage");

  const double max_s_weight = node.declare_parameter<double>(
    "obstacle_cruise.cruise_planning.optimization_based_planner.max_s_weight");
  const double max_v_weight = node.declare_parameter<double>(
    "obstacle_cruise.cruise_planning.optimization_based_planner.max_v_weight");
  const double over_s_safety_weight = node.declare_parameter<double>(
    "obstacle_cruise.cruise_planning.optimization_based_planner.over_s_safety_weight");
  const double over_s_ideal_weight = node.declare_parameter<double>(
    "obstacle_cruise.cruise_planning.optimization_based_planner.over_s_ideal_weight");
  const double over_v_weight = node.declare_parameter<double>(
    "obstacle_cruise.cruise_planning.optimization_based_planner.over_v_weight");
  const double over_a_weight = node.declare_parameter<double>(
    "obstacle_cruise.cruise_planning.optimization_based_planner.over_a_weight");
  const double over_j_weight = node.declare_parameter<double>(
    "obstacle_cruise.cruise_planning.optimization_based_planner.over_j_weight");
...
std::vector<TrajectoryPoint> OptimizationBasedPlanner::plan_cruise(
  ...
  [[maybe_unused]] std::optional<VelocityLimit> & velocity_limit)
{
  ...
  const size_t closest_idx =
    planner_data->find_segment_index(stop_traj_points, planner_data->current_odometry.pose.pose);
  ...
  a0 = std::min(common_param_.max_accel, std::max(a0, common_param_.min_accel));
  ...
  data.a0 = a0;
  data.v_max = v_max;
  data.a_max = common_param_.max_accel;
  data.a_min = common_param_.min_accel;
  data.j_max = common_param_.max_jerk;
  data.j_min = common_param_.min_jerk;
  data.limit_a_max = common_param_.limit_max_accel;
  data.limit_a_min = common_param_.limit_min_accel;
  data.limit_j_max = common_param_.limit_max_jerk;
  data.limit_j_min = common_param_.limit_min_jerk;
  data.t_dangerous = t_dangerous_;
  data.idling_time = cruise_planning_param_.idling_time;
...
{
  const auto & ego_vel = planner_data->current_odometry.twist.twist.linear.x;
  ...
  const auto current_closest_point = autoware::motion_utils::calcInterpolatedPoint(
    autoware::motion_utils::convertToTrajectory(input_traj_points),
  ...
  if (smoothed_trajectory_ptr_) {
    prev_output_closest_point = autoware::motion_utils::calcInterpolatedPoint(
      *smoothed_trajectory_ptr_, ego_pose, planner_data->ego_nearest_dist_threshold,
      planner_data->ego_nearest_yaw_threshold);
  } else {
    prev_output_closest_point = autoware::motion_utils::calcInterpolatedPoint(
      autoware::motion_utils::convertToTrajectory(prev_traj_points), ego_pose,
  ...
  if (vehicle_speed < engage_vel_thr) {
    if (target_vel >= engage_velocity_) {
      const auto stop_dist = autoware::motion_utils::calcDistanceToForwardStopPoint(
        input_traj_points, ego_pose, planner_data->ego_nearest_dist_threshold,
        planner_data->ego_nearest_yaw_threshold);
  ...
  const std::vector<TrajectoryPoint> & stop_traj_points)
{
  ...
  const auto closest_stop_dist = autoware::motion_utils::calcDistanceToForwardStopPoint(
    ...
    planner_data->ego_nearest_dist_threshold, planner_data->ego_nearest_yaw_threshold);
  if (
    closest_stop_dist && *closest_stop_dist < 0.5 &&
    planner_data->current_odometry.twist.twist.linear.x < 0.6) {
    return true;
  }
  ...
  const std::vector<CruiseObstacle> & obstacles, const std::vector<double> & time_vec)
{
  ...
  const auto traj_length = calcTrajectoryLengthFromCurrentPose(
    stop_traj_points, planner_data->current_odometry.pose.pose, planner_data);
  if (!traj_length) {
    return {};
  }
  ...
  for (size_t o_idx = 0; o_idx < obstacles.size(); ++o_idx) {
    ...
    const double rss_dist =
      calc_rss_distance(planner_data->current_odometry.twist.twist.linear.x, obj_vel);

    const auto & safe_distance_margin = cruise_planning_param_.safe_distance_margin;
    const double ego_obj_length = autoware::motion_utils::calcSignedArcLength(
      stop_traj_points, planner_data->current_odometry.pose.pose.position,
      obj.collision_points.front().point);
  ...
  if (min_slow_down_idx) {
    ...
    if (marker_pose) {
      ...
      // publish rviz marker
      auto loaned_wall_msg = debug_wall_marker_pub_->borrow_loaned_message();
      *loaned_wall_msg = wall_msg;
      debug_wall_marker_pub_->publish(std::move(loaned_wall_msg));
    }
  }
  ...
  const std::vector<double> & time_vec, const double traj_length)
{
  ...
  if (onEgoTrajectory) {
    ...
    return getSBoundariesForOnTrajectoryObject(
      stop_traj_points, planner_data, time_vec, safe_distance_margin, object, traj_length);
  }
  ...
  const double safety_distance, const CruiseObstacle & object, const double traj_length) const
{
  ...
  const double dist_to_collision_point = calc_distance_to_collisionPoint(
    traj_points, planner_data, object.collision_points.front().point);
  ...
  const double safety_distance, const CruiseObstacle & object, const double traj_length)
{
  ...
  for (const auto & collision_point : object.collision_points) {
    ...
    const double dist_to_collision_point =
      calc_distance_to_collisionPoint(traj_points, planner_data, collision_point.point);
  ...
  const polygon_utils::PointWithStamp & point)
{
  ...
  if ((rclcpp::Time(point.stamp) - clock_->now()).seconds() > 0.1) {
    return false;
  }
  ...
  if (lateral_offset < planner_data->vehicle_info_.max_lateral_offset_m + 0.1) {
    return true;
  }
...
std::optional<double> OptimizationBasedPlanner::calcTrajectoryLengthFromCurrentPose(
  ...
  const std::shared_ptr<const PlannerData> planner_data)
{
  const size_t ego_segment_idx = planner_data->find_segment_index(traj_points, ego_pose);
  ...
  const auto dist_to_closest_stop_point = autoware::motion_utils::calcDistanceToForwardStopPoint(
    traj_points, ego_pose, planner_data->ego_nearest_dist_threshold,
    planner_data->ego_nearest_yaw_threshold);
  ...
  const VelocityOptimizer::OptimizationResult & opt_result)
{
  ...
  {
    auto loaned_boundary_traj = boundary_pub_->borrow_loaned_message();
    loaned_boundary_traj->header.stamp = current_time;
    loaned_boundary_traj->points.resize(s_boundaries.size());
    double boundary_s_max = 0.0;
    for (size_t i = 0; i < s_boundaries.size(); ++i) {
      const double bound_s = s_boundaries.at(i).max_s;
      const double bound_t = time_vec.at(i);
      loaned_boundary_traj->points.at(i).pose.position.x = bound_s;
      loaned_boundary_traj->points.at(i).pose.position.y = bound_t;
      boundary_s_max = std::max(bound_s, boundary_s_max);
    }
    boundary_pub_->publish(std::move(loaned_boundary_traj));
  }

  {
    auto loaned_optimized_sv_traj = optimized_sv_pub_->borrow_loaned_message();
    loaned_optimized_sv_traj->header.stamp = current_time;
    loaned_optimized_sv_traj->points.resize(opt_result.s.size());
    for (size_t i = 0; i < opt_result.s.size(); ++i) {
      const double s = opt_result.s.at(i);
      const double v = opt_result.v.at(i);
      loaned_optimized_sv_traj->points.at(i).pose.position.x = s + offset;
      loaned_optimized_sv_traj->points.at(i).pose.position.y = v;
    }
    optimized_sv_pub_->publish(std::move(loaned_optimized_sv_traj));
  }

  {
    auto loaned_optimized_st_graph = optimized_st_graph_pub_->borrow_loaned_message();
    loaned_optimized_st_graph->header.stamp = current_time;
    loaned_optimized_st_graph->points.resize(opt_result.s.size());
    for (size_t i = 0; i < opt_result.s.size(); ++i) {
      const double bound_s = opt_result.s.at(i);
      const double bound_t = opt_result.t.at(i);
      loaned_optimized_st_graph->points.at(i).pose.position.x = bound_s;
      loaned_optimized_st_graph->points.at(i).pose.position.y = bound_t;
    }
    optimized_st_graph_pub_->publish(std::move(loaned_optimized_st_graph));
  }
}
}  // namespace autoware::motion_velocity_planner
