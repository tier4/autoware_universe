...
visualization_msgs::msg::MarkerArray RoadUserStopModule::createDebugMarkerArray() const
{
  ...
  if (params_.debug.publish_debug_markers) {
    ...
    if (debug_data_.stop_index) {
      ...
      // Note: stop position would need to be stored in debug_data_ for proper visualization
      // This is a placeholder that would need the actual stop position
...
visualization_msgs::msg::MarkerArray RoadUserStopModule::createLaneletPolygonsMarkerArray(
  const lanelet::ConstLanelets & lanelets, const std::string & ns,
