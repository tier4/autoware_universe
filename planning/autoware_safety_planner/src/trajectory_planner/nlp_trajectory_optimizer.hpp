...
#ifndef TRAJECTORY_PLANNER__NLP_TRAJECTORY_OPTIMIZER_HPP_
#define TRAJECTORY_PLANNER__NLP_TRAJECTORY_OPTIMIZER_HPP_
...
// それを三段フォールバック (B/C 行を段階的に落とす) と独立検証で埋める。
//
...
//
// コリドー (幾何の B/C 行) は **SSC の semantic corridor を共用**する。v1 にはステージごとに
// 半空間を彫る層が他に無く、cube (s,l,t) は「その時間帯の自由空間」を保証しているので、
// ステージ点をその cube に閉じ込める形で載せられる。NLP 専用のコリドー彫りを新設していない
// のは、非凸探索 = ホモトピー決定を 2 箇所に持たせないため。
...
struct NlpParams
{
  ...
  //! [m] コリドー面に対して追加で確保したい余裕 (C 幾何行)。Level 2 以降は 0 になる
  double comfort_clearance_m{0.5};
...
//! 1 ステージ分のコリドー。横 2 面 + 前方カット 1 面 (この順)。後方カットは課さない
...
//! そこでの中心線接線・法線で cube の 4 面を世界座標の直線に近似する
...
#endif  // TRAJECTORY_PLANNER__NLP_TRAJECTORY_OPTIMIZER_HPP_
