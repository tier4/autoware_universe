...
__host__ __device__ __forceinline__ bool checkRectSegmentIntersections(
    const float ego_x, const float ego_y, const float ego_yaw,
    const float front_ext, const float back_ext, 
    const float left_ext, const float right_ext,
    const float margin,
    const float* __restrict__ seg_x0, const float* __restrict__ seg_y0,
    const float* __restrict__ seg_x1, const float* __restrict__ seg_y1,
