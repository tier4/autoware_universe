  ...
  rclcpp::Node * node, const VehicleInfo & vehicle_info, const Params & params)
{
  ...
  pointcloud_filter_ = std::make_unique<obstacle_stop::PointCloudFilter>(
    ...
    params_.pointcloud.voxel_grid_filter.z, static_cast<int>(params_.pointcloud.voxel_grid_filter.min_size),
    params_.pointcloud.clustering.tolerance, static_cast<int>(params_.pointcloud.clustering.min_size),
...
void MrmObstacleStopPlanner::publish_debug_data(const std::string & ns) const
{
  ...
  auto add_polygon_marker = [&](
                              const Polygon2d & polygon, const std::string & marker_ns, const int id,
