...
DetectionAreaWithRTCModuleManager::DetectionAreaWithRTCModuleManager(rclcpp::Node & node)
: SceneModuleManagerInterfaceWithRTC(
    node, getModuleName(), getEnableRTC(node, std::string(getModuleName()) + ".enable_rtc"))
{
  const std::string ns(DetectionAreaWithRTCModuleManager::getModuleName());
...
void DetectionAreaWithRTCModuleManager::launchNewModules(
  const autoware_internal_planning_msgs::msg::PathWithLaneId & path)
{
  for (const auto & detection_area_with_lane_id :
    ...
    if (!isModuleRegistered(module_id)) {
      registerModule(std::make_shared<DetectionAreaWithRTCModule>(
        module_id, lane_id, *detection_area_with_lane_id.first, planner_param_,
        logger_.get_child("detection_area_with_rtc_module"), clock_, time_keeper_,
...
DetectionAreaWithRTCModuleManager::getModuleExpiredFunction(
  const autoware_internal_planning_msgs::msg::PathWithLaneId & path)
{
...
PLUGINLIB_EXPORT_CLASS(
  autoware::behavior_velocity_planner::DetectionAreaWithRTCModulePlugin,
