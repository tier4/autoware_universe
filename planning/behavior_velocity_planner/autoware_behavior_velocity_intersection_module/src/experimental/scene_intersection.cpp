...
bool IntersectionModule::can_smoothly_stop_at(
  const Trajectory & path, const double ego_s, const double stop_s,
  ...
  const std::vector<geometry_msgs::msg::Point> & right_bound, const PlannerData & planner_data)
{
  ...
  const auto [intersection_stoplines, path_lanelets] = prepare_data.ok();
  ...
  if (is_stuck_status) {
    if (can_smoothly_stop_at(path, closest_s, is_stuck_status->stuck_stopline_s, planner_data)) {
  ...
  const auto [occlusion_status, is_occlusion_cleared_with_margin, is_occlusion_state] =
    getOcclusionStatus(traffic_prioritized_level, path, planner_data);
  ...
  if (is_permanent_go_) {
    if (
      ...
      if (can_smoothly_stop_at(
            path, closest_s, intersection_stoplines.maximum_footprint_overshoot_line.value(),
            planner_data)) {
        ...
        const auto stop_line_s = closest_s;
        return NonOccludedCollisionStop{
          closest_s, stop_line_s, occlusion_stopline_s, occlusion_diag};
      }
    }
    if (has_collision) {
  ...
  const auto is_green_pseudo_collision_status =
    isGreenPseudoCollisionStatus(closest_s, collision_stopline_s, intersection_stoplines);
  if (is_green_pseudo_collision_status) {
    if (can_smoothly_stop_at(path, closest_s, collision_stopline_s, planner_data)) {
  ...
  // ==========================================================================================
  const auto yield_stuck_status = isYieldStuckStatus(path, intersection_stoplines, planner_data);
  if (yield_stuck_status) {
    if (can_smoothly_stop_at(path, closest_s, yield_stuck_status->stuck_stopline_s, planner_data)) {
  ...
  const bool collision_stop_feasible =
    can_smoothly_stop_at(path, closest_s, collision_stopline_s, planner_data);
  ...
  auto stoppedForDurationOrPassed = [&](const double s, StateMachine & state_machine) {
    ...
    const auto dist_stopline = s - closest_s;
  ...
  if (!stopped_at_default_line_or_passed) {
    if (can_smoothly_stop_at(path, closest_s, default_stopline_s, planner_data)) {
  ...
  const bool occlusion_stop_feasible =
    can_smoothly_stop_at(path, closest_s, occlusion_stopline_s, planner_data);
...
void prepareRTCByDecisionResult(
  [[maybe_unused]] const T & result, [[maybe_unused]] const Trajectory & path,
...
template <>
void prepareRTCByDecisionResult(
  [[maybe_unused]] const InternalError & result, [[maybe_unused]] const Trajectory & path,
  [[maybe_unused]] bool * default_safety, [[maybe_unused]] double * default_distance,
  [[maybe_unused]] bool * occlusion_safety, [[maybe_unused]] double * occlusion_distance)
{
}
...
template <>
void prepareRTCByDecisionResult(
  [[maybe_unused]] const OverPassJudge & result, [[maybe_unused]] const Trajectory & path,
  [[maybe_unused]] bool * default_safety, [[maybe_unused]] double * default_distance,
  [[maybe_unused]] bool * occlusion_safety, [[maybe_unused]] double * occlusion_distance)
{
}
...
template <>
void prepareRTCByDecisionResult(
  const StuckStop & result, const Trajectory & path, bool * default_safety,
...
template <>
void prepareRTCByDecisionResult(
  const YieldStuckStop & result, const Trajectory & path, bool * default_safety,
  double * default_distance, bool * occlusion_safety, [[maybe_unused]] double * occlusion_distance)
{
...
template <>
void prepareRTCByDecisionResult(
  const NonOccludedCollisionStop & result, const Trajectory & path, bool * default_safety,
...
template <>
void prepareRTCByDecisionResult(
  const FirstWaitBeforeOcclusion & result, const Trajectory & path, bool * default_safety,
  double * default_distance, bool * occlusion_safety, double * occlusion_distance)
{
...
template <>
void prepareRTCByDecisionResult(
  const PeekingTowardOcclusion & result, const Trajectory & path, bool * default_safety,
  double * default_distance, bool * occlusion_safety, double * occlusion_distance)
{
...
template <>
void prepareRTCByDecisionResult(
  const OccludedAbsenceTrafficLight & result, const Trajectory & path, bool * default_safety,
  double * default_distance, bool * occlusion_safety, double * occlusion_distance)
{
...
template <>
void prepareRTCByDecisionResult(
  const OccludedCollisionStop & result, const Trajectory & path, bool * default_safety,
  double * default_distance, bool * occlusion_safety, double * occlusion_distance)
{
...
template <>
void prepareRTCByDecisionResult(
  const Safe & result, const Trajectory & path, bool * default_safety, double * default_distance,
...
template <>
void prepareRTCByDecisionResult(
  const FullyPrioritized & result, const Trajectory & path, bool * default_safety,
  double * default_distance, bool * occlusion_safety, double * occlusion_distance)
{
...
void IntersectionModule::prepareRTCStatus(
  const DecisionResult & decision_result, const Trajectory & path)
{
  ...
  const IntersectionStopLines & intersection_stoplines, const PlannerData & planner_data)
{
  ...
  const auto pass_judge_line_s = [&]() {
    if (planner_param_.occlusion.enable) {
      ...
      if (is_occlusion_state && closest_s > original_pass_judge_line_s) {
