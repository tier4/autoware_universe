...
void IntersectionModule::updateObjectInfoManagerArea(const PlannerData & planner_data)
{
  ...
  for (const auto & predicted_object : planner_data.predicted_objects->objects) {
    ...
    const bool in_intersection_area = [&]() {
      ...
      return bg::within(autoware_utils::Point2d{obj_pos.x, obj_pos.y}, intersection_area.value());
    }();
...
{
  if (passed_judge_line_first_time) {
    object_info_manager_.setPassedPassJudgeLineFirstTime(clock_->now());
  }
  ...
  {
    object_ttc_time_array->stamp = clock_->now();
    ...
    object_ttc_time_array->layout.dim.at(0).size =
      1;  // incremented in the loop, first row is lane_id
    object_ttc_time_array->layout.dim.at(1).label =
      ...
      "end_time, end_dist, first_collision_x, first_collision_y, last_collision_x, "
      "last_collision_y, "
      "prd_x[0], ... pred_x[19], pred_y[0], ... pred_y[19]]";
    object_ttc_time_array->layout.dim.at(1).size = object_debug_size;
    for (unsigned i = 0; i < object_debug_size; ++i) {
  ...
  for (auto & object_info : object_info_manager_.attentionObjects()) {
    ...
    for (const auto & predicted_path_ptr : sorted_predicted_paths) {
      ...
      const auto object_passage_interval_opt =
        findPassageInterval(predicted_path, predicted_object.shape, ego_poly);
      ...
      auto get_object_info = [&]() {
        ...
        const auto [object_start_itr, object_end_itr] = object_passage_interval.interval_position;
        const auto & object_start_pos = object_passage_interval.path.at(object_start_itr).position;
        const auto & object_end_pos = object_passage_interval.path.at(object_end_itr).position;
        std::vector<double> debug;
        debug.reserve(object_debug_size);

        debug.insert(
          debug.end(),
          {pose.position.x, pose.position.y, tf2::getYaw(pose.orientation), shape.dimensions.x,
           shape.dimensions.y,
           predicted_object.kinematics.initial_twist_with_covariance.twist.linear.x,
           1.0 * static_cast<int>(collision_detected), object_enter_time, object_exit_time,
           ego_start_itr->first, ego_start_itr->second, ego_end_itr->first, ego_end_itr->second,
           object_start_pos.x, object_start_pos.y, object_end_pos.x, object_end_pos.y});
        for (unsigned i = 0; i < 20; ++i) {
          const auto & pos = object_passage_interval.path
                               .at(std::min<size_t>(i, object_passage_interval.path.size() - 1))
                               .position;
          debug.push_back(pos.x);
          debug.push_back(pos.y);
        }
        return debug;
      };
      ...
      if (collision_detected) {
        ...
        object_debug_info = get_object_info();
        break;
      }
      if (!safe_interval) {
        // ==========================================================================================
    ...
    if (passed_judge_line_first_time) {
      object_info->setDecisionAtPassJudgeLinePassage(
        CollisionKnowledge{
          clock_->now(),  // stamp
          unsafe_interval
    ...
    if (object_debug_info) {
      const auto & data = object_debug_info.value();
      object_ttc_time_array->layout.dim.at(0).size++;
      std::copy(data.begin(), data.end(), std::back_inserter(object_ttc_time_array->data));
    }
  }
}
...
{
  std::string diag;
  if (!safely_passed_judge_line_time_) {
    return diag;
  }
  const auto [passed_judge_line_time, passed_judge_line_pose] =
    safely_passed_judge_line_time_.value();
  const auto passed_judge_line_time_double =
    static_cast<double>(passed_judge_line_time.nanoseconds()) / 1e+9;
  ...
  // vargs
  for (const auto & object_info : too_late_detect_objects) {
    if (object_info->unsafe_interval()) {
      const auto & unsafe_interval = object_info->unsafe_interval().value();
      const double time_diff = now_double - passed_judge_line_time_double;
      diag += fmt::format(
        "object {0} was not detected when ego passed the pass judge line at {1}, but now at "
        "{2}, collision is detected after {3}~{4} seconds on the first attention lanelet of type\n",
        object_info->uuid_str,                // 0
        passed_judge_line_time_double,        // 1
        now_double,                           // 2
      ...
      if (past_position_opt) {
        ...
        diag += fmt::format(
          "this object is estimated to have been at x = {0}, y = {1} when ego passed the pass "
          "judge line({2} seconds before from now) given the estimated current velocity {3}[m/s]. "
          "ego was at x = {4}, y = {5} when it passed the pass judge line so it is the fault "
          "of detection side that failed to detect around {6}[m] range at that time.\n",
          past_position.x,                                                        // 0
          past_position.y,                                                        // 1
          time_diff,                                                              // 2
          object_info->observed_velocity(),                                       // 3
          passed_judge_line_pose.position.x,                                      // 4
          passed_judge_line_pose.position.y,                                      // 5
          autoware_utils::calc_distance2d(passed_judge_line_pose, past_position)  // 6
        );
      }
    }
  }
  for (const auto & object_info : misjudge_objects) {
    if (object_info->unsafe_interval() && object_info->decision_at_pass_judge_line_passage()) {
      const auto & decision_at_pass_judge_line =
        object_info->decision_at_pass_judge_line_passage().value();
      const auto decision_at_pass_judge_line_time =
        static_cast<double>(decision_at_pass_judge_line.stamp.nanoseconds()) / 1e+9;
      const auto & unsafe_interval = object_info->unsafe_interval().value();
      diag += fmt::format(
...
std::string IntersectionModule::generateEgoRiskEvasiveDiagnosis(
  const Trajectory & path, const double ego_s,
  ...
  const PlannerData & planner_data) const
{
  ...
  for (const auto & object_info : too_late_detect_objects) {
    ...
    const auto & p2 = unsafe_interval.path.at(end).position;
    const auto collision_pos =
      autoware_utils::create_point((p1.x + p2.x) / 2, (p1.y + p2.y) / 2, (p1.z + p2.z) / 2);
    ...
    const auto ego_dist_to_collision_pos = collision_s - ego_s;
...
IntersectionModule::CollisionStatus IntersectionModule::detectCollision(
  const bool is_over_pass_judge_line) const
{
  // ==========================================================================================
  ...
  std::vector<std::shared_ptr<ObjectInfo>> misjudge_objects;
  std::vector<std::shared_ptr<ObjectInfo>> too_late_detect_objects;
  for (const auto & object_info : object_info_manager_.attentionObjects()) {
    ...
    if (is_over_pass_judge_line) {
      const auto & decision_at_pass_judge_opt = object_info->decision_at_pass_judge_line_passage();
      if (!decision_at_pass_judge_opt) {
        too_late_detect_objects.emplace_back(object_info);
        debug_container = &debug_data_.too_late_detect_targets.objects;
      } else {
        const auto & decision_at_pass_judge = decision_at_pass_judge_opt.value();
        if (decision_at_pass_judge.safe_type != CollisionKnowledge::SafeType::UNSAFE) {
          misjudge_objects.emplace_back(object_info);
  ...
  return {collision_detected, too_late_detect_objects, misjudge_objects};
}
...
IntersectionModule::TimeDistanceArray IntersectionModule::calcIntersectionPassingTime(
  const Trajectory & path, const bool is_prioritized,
  ...
  const PlannerData & planner_data) const
{
  ...
  // last_intersection_stopline_candidate_s is used, which maybe almost-zero. at those almost-zero
  // velocity path points, ego future profile is almost "fixed" there.
  //
  // last_intersection_stopline_candidate_s must be carefully decided especially when ego
  // velocity is almost zero, because if last_intersection_stopline_candidate_s is at the
  // closest_s for example, ego is almost "fixed" at current position for the entire
  // spatiotemporal profile, which is judged as SAFE because that profile does not collide
  // with the predicted paths of objects.
  //
  // max(occlusion_stopline_s, first_attention_stopline_s) is used because
  // occlusion_stopline_s varies depending on the peeking offset parameter
  // ==========================================================================================
  ...
  // for ego's ttc
  const auto interval_with_objective_lane_id = util::findLaneIdsInterval(path, associative_ids_);
  if (!interval_with_objective_lane_id) {
    return {{0.0, 0.0}};  // has already passed the intersection.
  }
  const auto reference_path = [&]() {
    auto reference_path = autoware::experimental::trajectory::crop(
      path, closest_s, interval_with_objective_lane_id.value().end);
    if (!use_upstream_velocity) {
      reference_path.longitudinal_velocity_mps() = intersection_velocity;
    }
    return reference_path;
  }();
  const auto upstream_stopline = autoware::experimental::trajectory::search_zero_velocity_position(
    reference_path, last_intersection_stopline_candidate_s - closest_s);
  ...
  const auto reference_path_points = reference_path.restore();
  for (size_t i = 0; i + 1 < reference_path_points.size(); ++i) {
    const auto & p1 = reference_path_points[i];
    const auto & p2 = reference_path_points[i + 1];
    ...
    const double passing_velocity = [=]() {
      if (!use_upstream_velocity || !upstream_stopline || dist_sum < *upstream_stopline) {
  ...
  ego_ttc_array->data.reserve(time_distance_array.size() * ego_debug_size);
  for (unsigned i = 0; i < time_distance_array.size(); ++i) {
    ego_ttc_array->data.push_back(lane_id_);
  }
  for (const auto & [t, d] : time_distance_array) {
    ego_ttc_array->data.push_back(t);
  }
  for (const auto & [t, d] : time_distance_array) {
    ego_ttc_array->data.push_back(d);
  }
  for (const auto & p : reference_path_points) {
    ego_ttc_array->data.push_back(p.point.pose.position.x);
  }
  for (const auto & p : reference_path_points) {
    ego_ttc_array->data.push_back(p.point.pose.position.y);
  }
