...
bool VirtualTrafficLightModule::modifyPathVelocity(PathWithLaneId * _path)
{
  ...
  if (isBeforeStartLine(end_line_s)) {
  ...
  if (isAfterAnyEndLine(end_line_s)) {
  ...
  if (!virtual_traffic_light_state_) {
    ...
    insertStopVelocityAtStopLine(*path, end_line_s);
    updateInfrastructureCommand();
    return true;
  }
  ...
  if (isBeforeStopLine(end_line_s)) {
    setModuleState<State::REQUESTING>();
  ...
  if (!virtual_traffic_light_state_->is_finalized) {
    logInfoThrottle(
      ...
      map_data_.stop_line_id_for_log.c_str());
    insertStopVelocityAtEndLine(*path, end_line_s);

    if (isNearAnyEndLine(end_line_s) && planner_data_->isVehicleStopped()) {
...
bool VirtualTrafficLightModule::isBeforeStartLine(const double end_line_s) const
{
  ...
  const auto arc_length = calcArcLengthFromCollision(end_line_s, map_data_.start_line);
  return arc_length && *arc_length > 0;
}

bool VirtualTrafficLightModule::isBeforeStopLine(const double end_line_s) const
{
  // Since the module is registered, a collision should be detected usually.
  // Therefore if no collision found, vehicle's path is fully after the line.
  const auto arc_length = calcArcLengthFromCollision(end_line_s, *map_data_.stop_line);
  return arc_length && *arc_length > -planner_param_.dead_line_margin;
}

bool VirtualTrafficLightModule::isAfterAnyEndLine(const double end_line_s) const
{
  ...
  const auto arc_length = calcArcLengthFromCollision(end_line_s, map_data_.end_lines);
  return arc_length && *arc_length < -planner_param_.dead_line_margin;
}

bool VirtualTrafficLightModule::isNearAnyEndLine(const double end_line_s) const
{
  const auto arc_length = calcArcLengthFromCollision(end_line_s, map_data_.end_lines);
...
void VirtualTrafficLightModule::insertStopVelocityAtStopLine(
  Trajectory & path, const double end_line_s)
{
  const auto collision = findLastCollisionBeforeEndLine(path, *map_data_.stop_line, end_line_s);
  ...
  if (!collision) {
    ...
    const auto ego_s = experimental::trajectory::find_nearest_index(
      *module_data_.path, planner_data_->current_odometry->pose.position);
    ...
    if (stop_distance < planner_param_.hold_stop_margin_distance && is_stopped) {
      ...
      path.longitudinal_velocity_mps().range(*collision + offset, path.length()).set(0.0);
    }

    // for virtual wall
    stop_pose = path.compute(*collision + offset).point.pose;
  }
...
void VirtualTrafficLightModule::insertStopVelocityAtEndLine(
  Trajectory & path, const double end_line_s)
{
  const auto collision = findLastCollisionBeforeEndLine(path, map_data_.end_lines, end_line_s);
