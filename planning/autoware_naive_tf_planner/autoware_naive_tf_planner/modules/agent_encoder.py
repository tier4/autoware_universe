    ...
    def forward(self, data):
        ...
        if self.use_ego_history:
            ...
            if self.drop_path == 0.0 or not self.training:
                ...
                shape = (ego_history_feature.shape[0],) + (1,) * (
                    ego_history_feature.ndim - 1
                )
                random_tensor = self.keep_prob + torch.rand(
                    shape,
                    dtype=ego_history_feature.dtype,
                    device=ego_history_feature.device,
                )
                random_tensor.floor_()  # binarize

                ego_history_feature = ego_history_feature.div(self.keep_prob) * random_tensor

            objs_feature = torch.cat(
                [ego_history_feature, objs_feature], dim=1
            )  # [B, N_obj + 1, dim]
