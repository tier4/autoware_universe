...
private:
  ...
  static std::pair<std::vector<float>, std::map<int32_t, std::vector<float>>>
  postprocess_trajectories(const std::vector<float> & ego_fut_preds, int32_t cmd)
  {
    std::vector<float> selected_trajectory(
      ego_fut_preds.begin() + cmd * 12, ego_fut_preds.begin() + (cmd + 1) * 12);
    ...
    for (int32_t command_idx = 0; command_idx < 3; command_idx++) {
      std::vector<float> trajectory(
        ego_fut_preds.begin() + command_idx * 12, ego_fut_preds.begin() + (command_idx + 1) * 12);
