  ...
  const CandidateTrajectory & candidate_trajectory, const FilterContext & context)
{
  ...
  const auto drac_artifact = collision_timing_assessment::assess(
    ego_trajectory_cache, candidate_trajectory.turn_indicators_command, *filtered_context.odometry,
    *filtered_context.predicted_objects, stop_tracker_, drac_param_map_, global_params_);
