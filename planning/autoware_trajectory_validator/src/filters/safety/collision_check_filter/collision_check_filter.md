...
* TTC+PET 計画速度で進行した際に、制動距離内でニアミスするリスクがあるかを判定
...
## TTC+PET 計画通りに進行した際に、制動距離内でニアミスするリスクがあるかを判定

自車が計画通りの経路や速度で進行した際に、制動距離進む時間内に他車とニアミスするリスクがあるかを判定する処理です。
「制動距離進む時間内」をTTCの値により、「ニアミス」の判定をPETの値により判定しているため、「TTC+PET」という命名を行っています。
...
| `ego_assumed_acceleration` | どれぐらい将来の事象まで判定対象とするか、すなわちTTCの上限を決める値 TTC+PETでは減速挙動を行った際のPETを評価しているわけではないことに注意 | -4.0 [m/s²] | J6の最大減速度 -6.0より、いくらかのマージンをとった値 少なくとも-4.0よりも強い減速度を設定することは想定していない |
