...
| --- | --- | --- |
| map_based | map_basaed_predictionという外部ノードで生成された情報を読み取っています <br> 自動車など：無理なく追従可能な全ての車線に対して、車線に追従するような経路が生成されます <br> 歩行者：現在の進行方向に向かう経路、もしくは横断歩道に吸い付くような経路が生成されます | 等速度運動（4/30時点では無し）、もしくは一定減速度運動での速度軌跡を内部で生成しています |
| constant_curvature | 物体が速度と角速度を維持する、すなわち一定曲率の経路を生成します  | 等速度運動（4/30時点では無し）、もしくは一定減速度運動での速度軌跡を内部で生成しています |
| diffusion_based | MLで生成された経路を読み取っています | MLで生成された速度情報の読み取り、及び内部での一定減速度運動での速度軌跡を生成の両方を行っています |
...
| `assessment_trajectories` | どの経路予測手法を用いるかを選択 複数選択可能 | map_based, constant_curvature, diffusion_based | とりあえず全部 |
...
## PET+TTC 計画通りに進行した際に、制動距離内でニアミスするリスクがあるかを判定

自車が計画通りの経路や速度で進行した際に、制動距離進む時間内に他車とニアミスするリスクがあるかを判定する処理です。
「ニアミス」の判定をPETの値により、「制動距離進む時間内」をTTCの値により判定しているため、「PET+TTC」という命名を行っています。

※注記：
TTCが値を持つ、すなわち衝突する場合には、PETの値は必ず0になります。
そのため、「TTCのみで危険とできるような状況はPETのみでも危険と判定される」ということが、TTCやPETの閾値によらずに言えます。
...
* **3. 危険判定と対処：** 算出されたPETが許容マージン（閾値）を下回る場合はニアミスのリスクが高いと判断し、該当する経路を危険とみなし棄却・除外する。場合によっては、-6.0 m/s² の緊急停止を行う。
...
| `enable_assessment` | 評価計算を行うかを選択 | true |  |
| `assessment_trajectories` | どの経路予測手法を用いるかを選択 複数選択可能 | map_based, constant_curvature, diffusion_based | とりあえず全部 |
| `ego_total_braking_delay` | どれぐらい将来の事象まで判定対象とするか、すなわちTTCの上限を決める値  safety_filter以後の遅延時間 [s] 認識側の遅延は含まない | 0.4 | 以下の合計より、設定  *autoware遅延0.1s  *車両遅延 0.2s *ジャークによる遅延 0.1s （0.4/20/2より） |
| `ego_assumed_acceleration` | どれぐらい将来の事象まで判定対象とするか、すなわちTTCの上限を決める値 PET+TTCでは減速挙動を行った際のPETを評価しているわけではないことに注意 | -4.0 [m/s²] | J6の最大減速度 -6.0より、いくらかのマージンをとった値 少なくとも-4.0よりも強い減速度を設定することは想定していない |
| `warn_threshold.ego_first_passing_time_gap` | 開発用 | 1.0 |  |
| `warn_threshold.object_first_passing_time_gap` | 開発用 | 1.0 |  |
| `error_threshold.ego_first_passing_time_gap` |  | 0.6 |  |
| `error_threshold.object_first_passing_time_gap` |  | 0.3 |  |
...
for (各物体軌跡 in 全ての物体の評価したい予測経路) {
    ...
    // 2. 自車と物体が「ほぼ同じ場所を、どれくらいの危険な時間差で通るか」を調べる
