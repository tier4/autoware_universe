  ...
  const TrajectoryPoints & traj_points, const FilterContext & context)
{
  ...
  const auto front_trajectory = autoware::experimental::trajectory::add_offset(
  ...
  const auto first_stop_distance =
    stopping_intervals.empty() ? std::optional<double>{}
                               : std::make_optional(stopping_intervals.front().start.distance);

  double checked_distance = std::min(temporal_trajectory.length(), max_trajectory_length);
  if (first_stop_distance) {
    checked_distance = std::min(checked_distance, *first_stop_distance);
  }
  const auto stop_distance = first_stop_distance && *first_stop_distance <= checked_distance
                               ? first_stop_distance
                               : std::nullopt;

  std::optional<lanelet::BasicPoint2d> stop_point;
  if (stop_distance) {
    const auto point = temporal_trajectory.compute_from_distance(*stop_distance);
    stop_point = lanelet::BasicPoint2d{point.pose.position.x, point.pose.position.y};
  }

  const auto first_crossing_before = [&front_trajectory, checked_distance](
                                       const lanelet::BasicLineString2d & stop_line) {
    const auto crossings = autoware::experimental::trajectory::crossed(front_trajectory, stop_line);
    const auto it = std::find_if(
      crossings.begin(), crossings.end(),
      [checked_distance](const auto & p) { return p.distance <= checked_distance; });
    return it == crossings.end() ? std::nullopt : std::make_optional(*it);
  };
  ...
  for (const auto & red_stop_line : red_stop_lines) {
    if (first_crossing_before(red_stop_line)) {
  ...
  for (const auto & amber_stop_line : amber_stop_lines) {
    const auto crossing = first_crossing_before(amber_stop_line);
    if (!crossing) {
      continue;
    }
    ...
    if (!can_pass_amber_light(
          crossing->distance, current_velocity, current_acceleration, crossing->time)) {
