  ...
  if (mpt_param_.enable_manual_warm_start) {
    ...
    if (prev_trajs && prev_trajs->ref_points.size() > 1) {
      ...
      for (size_t i = 0; i + 1 < N_ref; ++i) {
        ...
        const size_t prev_N_ref = prev_trajs->ref_points.size();
        if (prev_idx + 2 > prev_N_ref) {
          prev_idx = static_cast<int>(prev_N_ref) - 2;
          offset = 0.5;
        }

        const double prev_val = prev_trajs->ref_points.at(prev_idx).optimized_input;
        const double next_val = prev_trajs->ref_points.at(prev_idx + 1).optimized_input;
