  ...
  context_{std::make_shared<TrajectoryModifierContext>(this)}
{
  ...
  trajectories_sub_ = create_subscription<CandidateTrajectories>(
    ...
    std::bind(&TrajectoryModifier::on_trajectories, this, std::placeholders::_1));
  ...
  load_plugins();
  RCLCPP_INFO(get_logger(), "TrajectoryModifier initialized with %zu plugins", plugins_.size());
}
...
void TrajectoryModifier::load_plugins()
{
  plugins_.clear();
...
void TrajectoryModifier::load_plugin(
  const std::string & class_name, const std::size_t pipeline_index)
{
  try {
    auto modifier_plugin = plugin_loader_.createSharedInstance(class_name);
    const auto instance_name = class_name + "#" + std::to_string(pipeline_index);
    modifier_plugin->initialize(class_name, instance_name, this, time_keeper_, context_, params_);
    plugins_.push_back(std::move(modifier_plugin));
    RCLCPP_INFO(
      get_logger(), "Loaded trajectory modifier plugin '%s' as '%s'", class_name.c_str(),
      instance_name.c_str());
  } catch (const pluginlib::PluginlibException & error) {
    RCLCPP_ERROR(
      get_logger(), "Failed to load trajectory modifier plugin '%s': %s", class_name.c_str(),
      error.what());
  }
}
...
tl::expected<TrajectoryModifierData, std::string> TrajectoryModifier::make_input_data()
{
...
void TrajectoryModifier::publish_processing_time(const double processing_time_ms)
{
  debug_publisher_->publish<autoware_internal_debug_msgs::msg::Float64Stamped>(
    "processing_time_ms", processing_time_ms);
}

void TrajectoryModifier::on_trajectories(const CandidateTrajectories::ConstSharedPtr msg)
{
  autoware_utils_debug::ScopedTimeTrack st(__func__, *time_keeper_);
