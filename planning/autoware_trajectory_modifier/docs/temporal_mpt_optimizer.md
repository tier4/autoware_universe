...
1. Add the plugin class to `plugin_names` in `config/trajectory_modifier.param.yaml` (position sets when it runs relative to other plugins).
...
Plugin-specific parameters live in `config/plugins/trajectory_temporal_mpt_optimizer.param.yaml` (loaded from `launch/trajectory_modifier.launch.xml`).
...
```bash
ros2 run autoware_trajectory_modifier temporal_mpt_debug_visualizer.py
```
