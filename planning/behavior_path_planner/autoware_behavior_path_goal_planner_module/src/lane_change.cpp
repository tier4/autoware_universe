 ...
 *          route using RouteHandler::getNextLaneletWithinRoute(). Always returns at least
 *          current_target_lane_id.
 * @param current_target_lane_id Current target lane ID (always included in result)
 * @param goal_lane_id Goal lane ID to search for
 * @param lanelet_map Lanelet map for lane lookup
 * @param route_handler Route handler for traversing route
  ...
  const autoware::route_handler::RouteHandler & route_handler)
{
  ...
  while (route_handler.getNextLaneletWithinRoute(current_lanelet, &next_lanelet)) {
    ...
    // Termination condition 1: Loop detection
    if (visited.find(next_id) != visited.end()) break;

    visited.insert(next_id);
    result.push_back(next_id);

    // Termination condition 2: Goal found
    if (next_id == goal_lane_id) return result;

    current_lanelet = next_lanelet;
  }

  // Termination condition 3: No more following lanes, goal not found
  // Return partial result (at least current_target_lane_id is included)
  return result;
}
}  // namespace
