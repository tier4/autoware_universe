  ...
  const std::optional<rclcpp::Time> & selected_time, const GoalPlannerParameters & parameters)
{
  ...
  return !goal_planner_utils::is_on_modified_goal(current_pose, modified_goal, parameters) &&
         has_enough_time_passed;
}
...
void LaneParkingPlanner::onTimer()
{
  ...
  const bool need_update = std::invoke([&]() {
    ...
    return goal_planner_utils::should_regenerate_path_candidates(
             local_planner_data->self_odometry->pose.pose, upstream_module_output,
             original_upstream_module_output_, lane_change_status_changed_since_last_wakeup)
      .should_regenerate;
  });
  if (!need_update) {
    return;
  ...
  // clear existing candidates when starting regeneration
  {
    std::lock_guard<std::mutex> guard(mutex_);
    response_.pull_over_path_candidates.clear();
    RCLCPP_INFO(getLogger(), "clearing pull over path candidates for regeneration");
  }
  ...
  RCLCPP_INFO(
    getLogger(), "LaneParkingPlanner generated %lu pull over path candidates",
  ...
  // set response
  {
    ...
    response_.original_upstream_module_output = upstream_module_output;
  }
}
...
{
  autoware_utils::StopWatch timer;
  timer.tic("bezier");
  std::vector<PullOverPath> path_candidates_all;
  // print goal_candidates.size()
  RCLCPP_INFO(
    getLogger(), "Trying to generate bezier paths for %lu goal candidates", goal_candidates.size());
...
void GoalPlannerModule::updateData()
{
  ...
  if (
    ...
    if (self_to_goal_arc_length < parameters_.pull_over_prepare_length) {
      ...
      RCLCPP_INFO(
        getLogger(),
        "[%s] start preparing pull over path candidates once for goal ahead of %f meter",
        magic_enum::enum_name(getCurrentStatus()).data(), self_to_goal_arc_length);
      return;
    }
  }
  ...
  if (getCurrentStatus() == ModuleStatus::IDLE) {
    ...
    if (
      ...
      if (!result.reason.empty()) {
        RCLCPP_INFO_THROTTLE(
          getLogger(), *clock_, 5000, "[IDLE] restart preparing pull over path candidates: %s",
          result.reason.c_str());
      }
  ...
  // print lane_parking_response pull over path candidates size
  RCLCPP_INFO(
    getLogger(), "Lane parking response has %lu pull over path candidates",
    lane_parking_response.pull_over_path_candidates.size());
  ...
  if (context_data_) {
    if (should_update_lane_parking || lane_parking_response.pull_over_path_candidates.empty()) {
      // NOT_DECIDED: update both lane_parking and freespace_parking responses
      // debug print
      RCLCPP_INFO(getLogger(), "Updating lane_parking and freespace_parking responses");
      ...
      RCLCPP_INFO(getLogger(), "Updating only freespace_parking response");
      context_data_.value().update(
        new_decision_state.is_stable_safe, static_target_objects, dynamic_target_objects,
        prev_decision_state,
        isStopped(
    ...
    RCLCPP_INFO(getLogger(), "Creating new ContextData");
...
bool GoalPlannerModule::isExecutionRequested() const
{
  ...
  const Pose previous_terminal_vehicle_edge_pose = calc_offset_pose(
  ...
  PullOverContextData & context_data, const std::string & detail)
{
  ...
  if (context_data.lane_parking_response.pull_over_path_candidates.empty()) {
    RCLCPP_INFO_THROTTLE(
      getLogger(), *clock_, 5000,
      " [pull_over] planPullOverAsCandidate() is called without pull over path candidates");
    // Clear selected path since candidates are being regenerated
  ...
  const lanelet::ConstLanelets & target_lanes, const ObjectsFilteringParams & params)
{
  ...
  for (const auto & target_lane : target_lanes) {
    ...
    for (const auto & filtered_object : filtered_objects.objects) {
      const auto object_bbox = autoware_utils::to_polygon2d(filtered_object);
...
void GoalPlannerModule::setDebugData(const PullOverContextData & context_data)
{
  ...
  if (context_data.pull_over_path_opt) {
    ...
    autoware_utils::append_marker_array(
      goal_planner_utils::createLaneletPolygonMarkerArray(
        ...
        autoware_utils::create_marker_color(1.0, 0.7, 0.0, 0.999)),
      &debug_marker_);
  ...
  // visualize safety status maker
  {
    ...
    const auto color = prev_data.is_stable_safe ? create_marker_color(1.0, 1.0, 1.0, 0.99)
                                                : create_marker_color(1.0, 0.0, 0.0, 0.99);
  ...
  // Visualize planner type text
  {
    ...
    const auto color = context_data.pull_over_path_opt ? create_marker_color(1.0, 1.0, 1.0, 0.99)
                                                       : create_marker_color(1.0, 0.0, 0.0, 0.99);
    auto marker = create_default_marker(
