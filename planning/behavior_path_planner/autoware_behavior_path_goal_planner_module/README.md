...
| lateral_acceleration_threshold          | [m/s^2] | double | maximum allowable lateral acceleration near the path start. Used to filter out path candidates with excessive lateral acceleration. Paths where the yaw change or lateral deviation at a point `velocity × lateral_acceleration_filtering_duration` ahead exceeds this limit are rejected.                                                                                                                                                                       | 2.0           |
| lateral_acceleration_filtering_duration | [s]     | double | time duration from the start pose used to evaluate lateral acceleration. The check evaluates the path geometry at a point calculated as `velocity × duration` ahead of the start pose. **Note:** This check does NOT evaluate lateral acceleration at every point along the path during this duration. Instead, it only evaluates the lateral acceleration at the single endpoint by approximating the path as a circular arc from the start pose to that point. | 1.0           |
