...
- **Map-Based Activation**: Activates only when `direction_change_area` is set to `"yes"` in lanelet map
...
- **A `direction_change_area` attribute is set to `"yes"` in the lanelet map** (required)
- The path is within lanelets containing this tag

### Map Requirements

**Important**: This module only activates when lanelets in the path have the `direction_change_area` attribute set to `"yes"` in the Lanelet2 map. Without this, the module will not activate.

To enable the module:
1. Add the `direction_change_area` attribute to relevant lanelets in your Lanelet2 map
2. Set the attribute value to `"yes"` (e.g., `direction_change_area: "yes"`)

The module modifies the path by:
- Checking for `direction_change_area: "yes"` in lanelets along the path
