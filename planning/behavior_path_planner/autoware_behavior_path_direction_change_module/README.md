...
- **Map-Based Activation**: Activates only when `direction_change_area` tag is present in lanelet map
...
- **A `direction_change_area` tag is present in the lanelet map** (required)
- The path is within lanelets containing this tag

### Map Requirements

**Important**: This module only activates when lanelets in the path have the `direction_change_area` attribute/tag set in the Lanelet2 map. Without this tag, the module will not activate.

To enable the module:
1. Add the `direction_change_area` attribute to relevant lanelets in your Lanelet2 map
2. The attribute value can be any non-empty string (e.g., `direction_change_area: "true"` or `direction_change_area: "direction_change"`)

The module modifies the path by:
- Checking for `direction_change_area` tag in lanelets along the path
