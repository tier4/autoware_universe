...
bool is_ego_stopped(
  const TrajectoryOptimizerData & data, const double stopped_velocity_threshold_mps)
{
  ...
  const double stopped_trajectory_max_length_m)
{
  ...
  if (
    autoware::motion_utils::calcSignedArcLength(
      traj_points, traj_points.front().pose.position, traj_points.size() - 1) >
...
bool is_near_route_goal(
  const TrajectoryOptimizerData & data, const double goal_steer_zero_distance_m)
{
  if (!data.route) {
    return false;
  }
...
void TrajectoryTimeSequenceRawOptimizer::initialize(
...
void TrajectoryTimeSequenceRawOptimizer::set_up_params()
{
  auto * node_ptr = get_node_ptr();
  using autoware_utils_rclcpp::get_or_declare_parameter;

  opt_params_.weight_longitudinal =
    get_or_declare_parameter<double>(*node_ptr, "time_sequence_raw_optimizer.weight_longitudinal");
  opt_params_.weight_lateral =
    get_or_declare_parameter<double>(*node_ptr, "time_sequence_raw_optimizer.weight_lateral");
  opt_params_.weight_yaw =
    get_or_declare_parameter<double>(*node_ptr, "time_sequence_raw_optimizer.weight_yaw");
  opt_params_.weight_jerk =
    get_or_declare_parameter<double>(*node_ptr, "time_sequence_raw_optimizer.weight_jerk");
  opt_params_.weight_steering_rate =
    get_or_declare_parameter<double>(*node_ptr, "time_sequence_raw_optimizer.weight_steering_rate");
  opt_params_.terminal_weight_scale = get_or_declare_parameter<double>(
    *node_ptr, "time_sequence_raw_optimizer.terminal_weight_scale");
  opt_params_.min_velocity_mps =
    get_or_declare_parameter<double>(*node_ptr, "time_sequence_raw_optimizer.min_velocity_mps");
  opt_params_.max_velocity_mps =
    get_or_declare_parameter<double>(*node_ptr, "time_sequence_raw_optimizer.max_velocity_mps");
  opt_params_.min_acceleration_mps2 = get_or_declare_parameter<double>(
    *node_ptr, "time_sequence_raw_optimizer.min_acceleration_mps2");
  opt_params_.max_acceleration_mps2 = get_or_declare_parameter<double>(
    *node_ptr, "time_sequence_raw_optimizer.max_acceleration_mps2");
  opt_params_.min_jerk_mps3 =
    get_or_declare_parameter<double>(*node_ptr, "time_sequence_raw_optimizer.min_jerk_mps3");
  opt_params_.max_jerk_mps3 =
    get_or_declare_parameter<double>(*node_ptr, "time_sequence_raw_optimizer.max_jerk_mps3");
  opt_params_.max_steering_rate_rps = get_or_declare_parameter<double>(
    *node_ptr, "time_sequence_raw_optimizer.max_steering_rate_rps");
  opt_params_.max_lateral_acceleration_mps2 = get_or_declare_parameter<double>(
    *node_ptr, "time_sequence_raw_optimizer.max_lateral_acceleration_mps2");
  opt_params_.max_sqp_iterations =
    get_or_declare_parameter<int>(*node_ptr, "time_sequence_raw_optimizer.max_sqp_iterations");

  publish_debug_topics_ =
    get_or_declare_parameter<bool>(*node_ptr, "time_sequence_raw_optimizer.publish_debug_topics");
  use_stamped_ego_state_ =
    get_or_declare_parameter<bool>(*node_ptr, "time_sequence_raw_optimizer.use_stamped_ego_state");
  ego_state_buffer_duration_s_ = get_or_declare_parameter<double>(
    *node_ptr, "time_sequence_raw_optimizer.ego_state_buffer_duration_s");
  max_ego_stamp_mismatch_s_ = get_or_declare_parameter<double>(
    *node_ptr, "time_sequence_raw_optimizer.max_ego_stamp_mismatch_s");
  ego_buffer_.set_duration(ego_state_buffer_duration_s_);
  stopped_velocity_threshold_mps_ = get_or_declare_parameter<double>(
    *node_ptr, "time_sequence_raw_optimizer.stopped_velocity_threshold_mps");
  stopped_trajectory_max_length_m_ = get_or_declare_parameter<double>(
    *node_ptr, "time_sequence_raw_optimizer.stopped_trajectory_max_length_m");
  goal_steer_zero_enable_ =
    get_or_declare_parameter<bool>(*node_ptr, "time_sequence_raw_optimizer.goal_steer_zero_enable");
  goal_steer_zero_distance_m_ = get_or_declare_parameter<double>(
    *node_ptr, "time_sequence_raw_optimizer.goal_steer_zero_distance_m");
  goal_steer_zero_requires_stopped_ = get_or_declare_parameter<bool>(
    *node_ptr, "time_sequence_raw_optimizer.goal_steer_zero_requires_stopped");

  border_params_.enable = get_or_declare_parameter<bool>(*node_ptr, "road_border_avoidance.enable");
  border_params_.footprint_margin_m =
    get_or_declare_parameter<double>(*node_ptr, "road_border_avoidance.footprint_margin_m");
  border_params_.search_radius_m =
    get_or_declare_parameter<double>(*node_ptr, "road_border_avoidance.search_radius_m");
  border_params_.shift_step_m =
    get_or_declare_parameter<double>(*node_ptr, "road_border_avoidance.shift_step_m");
  border_params_.max_lateral_shift_m =
    get_or_declare_parameter<double>(*node_ptr, "road_border_avoidance.max_lateral_shift_m");
  border_params_.propagate_shift =
    get_or_declare_parameter<bool>(*node_ptr, "road_border_avoidance.propagate_shift");
  road_border_enable_ = border_params_.enable;
}

rcl_interfaces::msg::SetParametersResult TrajectoryTimeSequenceRawOptimizer::on_parameter(
  const std::vector<rclcpp::Parameter> & parameters)
{
  using autoware_utils_rclcpp::update_param;
  update_param(
    parameters, "time_sequence_raw_optimizer.weight_longitudinal", opt_params_.weight_longitudinal);
  update_param(
    parameters, "time_sequence_raw_optimizer.weight_lateral", opt_params_.weight_lateral);
  update_param(parameters, "time_sequence_raw_optimizer.weight_yaw", opt_params_.weight_yaw);
  update_param(parameters, "time_sequence_raw_optimizer.weight_jerk", opt_params_.weight_jerk);
  update_param(
    parameters, "time_sequence_raw_optimizer.weight_steering_rate",
    opt_params_.weight_steering_rate);
  update_param(
    parameters, "time_sequence_raw_optimizer.terminal_weight_scale",
    opt_params_.terminal_weight_scale);
  update_param(
    parameters, "time_sequence_raw_optimizer.min_velocity_mps", opt_params_.min_velocity_mps);
  update_param(
    parameters, "time_sequence_raw_optimizer.max_velocity_mps", opt_params_.max_velocity_mps);
  update_param(
    parameters, "time_sequence_raw_optimizer.min_acceleration_mps2",
    opt_params_.min_acceleration_mps2);
  update_param(
    parameters, "time_sequence_raw_optimizer.max_acceleration_mps2",
    opt_params_.max_acceleration_mps2);
  update_param(parameters, "time_sequence_raw_optimizer.min_jerk_mps3", opt_params_.min_jerk_mps3);
  update_param(parameters, "time_sequence_raw_optimizer.max_jerk_mps3", opt_params_.max_jerk_mps3);
  update_param(
    parameters, "time_sequence_raw_optimizer.max_steering_rate_rps",
    opt_params_.max_steering_rate_rps);
  update_param(
    parameters, "time_sequence_raw_optimizer.max_lateral_acceleration_mps2",
    opt_params_.max_lateral_acceleration_mps2);
  update_param(
    parameters, "time_sequence_raw_optimizer.max_sqp_iterations", opt_params_.max_sqp_iterations);
  update_param(
    parameters, "time_sequence_raw_optimizer.publish_debug_topics", publish_debug_topics_);
  update_param(
    parameters, "time_sequence_raw_optimizer.use_stamped_ego_state", use_stamped_ego_state_);
  update_param(
    parameters, "time_sequence_raw_optimizer.ego_state_buffer_duration_s",
    ego_state_buffer_duration_s_);
  update_param(
    parameters, "time_sequence_raw_optimizer.max_ego_stamp_mismatch_s", max_ego_stamp_mismatch_s_);
  update_param(
    parameters, "time_sequence_raw_optimizer.stopped_velocity_threshold_mps",
    stopped_velocity_threshold_mps_);
  update_param(
    parameters, "time_sequence_raw_optimizer.stopped_trajectory_max_length_m",
    stopped_trajectory_max_length_m_);
  update_param(
    parameters, "time_sequence_raw_optimizer.goal_steer_zero_enable", goal_steer_zero_enable_);
  update_param(
    parameters, "time_sequence_raw_optimizer.goal_steer_zero_distance_m",
    goal_steer_zero_distance_m_);
  update_param(
    parameters, "time_sequence_raw_optimizer.goal_steer_zero_requires_stopped",
    goal_steer_zero_requires_stopped_);
  update_param(parameters, "road_border_avoidance.enable", border_params_.enable);
  update_param(
    parameters, "road_border_avoidance.footprint_margin_m", border_params_.footprint_margin_m);
  update_param(parameters, "road_border_avoidance.search_radius_m", border_params_.search_radius_m);
  update_param(parameters, "road_border_avoidance.shift_step_m", border_params_.shift_step_m);
  update_param(
    parameters, "road_border_avoidance.max_lateral_shift_m", border_params_.max_lateral_shift_m);
  update_param(parameters, "road_border_avoidance.propagate_shift", border_params_.propagate_shift);
  road_border_enable_ = border_params_.enable;
  ego_buffer_.set_duration(ego_state_buffer_duration_s_);
  optimizer_.reset();
...
void TrajectoryTimeSequenceRawOptimizer::ingest_live_ego(const TrajectoryOptimizerData & data)
{
...
void TrajectoryTimeSequenceRawOptimizer::ensure_optimizer()
{
  ...
  optimizer_ =
    std::make_unique<time_sequence_raw::TrajectoryOptimizer>(opt_params_, vehicle_info_, 8);
  road_border_avoidance_ =
    std::make_unique<time_sequence_raw::RoadBorderAvoidance>(border_params_, vehicle_info_);
}

void TrajectoryTimeSequenceRawOptimizer::maybe_update_map(const TrajectoryOptimizerData & data)
{
...
void TrajectoryTimeSequenceRawOptimizer::publish_debug_data() const
{
  if (!publish_debug_topics_) {
    return;
  }
...
TrajectoryTimeSequenceRawOptimizer::resolve_steer_stop_mode(
  const autoware_planning_msgs::msg::Trajectory & reference, const TrajectoryOptimizerData & data)
{
