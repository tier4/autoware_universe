...
#ifndef AUTOWARE__TRAJECTORY_OPTIMIZER__TIME_SEQUENCE_RAW__STAMPED_EGO_BUFFER_HPP_
#define AUTOWARE__TRAJECTORY_OPTIMIZER__TIME_SEQUENCE_RAW__STAMPED_EGO_BUFFER_HPP_
...
namespace autoware::trajectory_optimizer::time_sequence_raw
{
...
private:
  ...
  void prune_locked(const rclcpp::Time & newest)
  {
    const auto drop_older_than = [&](auto & buf) {
      while (buf.size() > 1) {
        const auto age = (newest.nanoseconds() == 0) ? duration_s_ + 1.0
    ...
    const rclcpp::Time & query) const
  {
    ...
    out.steering_tire_angle = static_cast<float>(
      a->steering_tire_angle + alpha * (b->steering_tire_angle - a->steering_tire_angle));
    return out;
  }
...
#endif  // AUTOWARE__TRAJECTORY_OPTIMIZER__TIME_SEQUENCE_RAW__STAMPED_EGO_BUFFER_HPP_
