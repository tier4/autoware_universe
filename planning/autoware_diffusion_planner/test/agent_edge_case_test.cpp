...
TEST_F(AgentEdgeCaseTest, HdpKeepsPolygonObjectsWhileLegacyPathDropsThem)
{
  TrackedObjects objects;
  objects.header.stamp.sec = 1;
  objects.objects.push_back(tracked_object_);
  objects.objects.front().shape.type = autoware_perception_msgs::msg::Shape::POLYGON;

  AgentData legacy_agent_data;
  legacy_agent_data.update_histories(objects);
  EXPECT_TRUE(
    legacy_agent_data.transformed_and_trimmed_histories(Eigen::Matrix4d::Identity(), 1).empty());

  AgentData hdp_agent_data;
  hdp_agent_data.update_histories(objects, true);
  EXPECT_EQ(
    hdp_agent_data.transformed_and_trimmed_histories(Eigen::Matrix4d::Identity(), 1).size(), 1U);
}
