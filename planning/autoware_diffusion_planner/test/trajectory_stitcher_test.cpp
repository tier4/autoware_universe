...
protected:
  ...
  // time_from_start = 0.1 * (i + 1). The t=0 anchor (planning origin) is at x = 0.
  Trajectory make_trajectory(const size_t num_points = 20) const
  {
    Trajectory trajectory;
    trajectory.header.stamp = t0_;
  ...
  void store_default_trajectory() const
  {
    stitcher_->set_previous_trajectory(make_trajectory(), make_pose(0.0), uuid_a_);
  }
...
TEST_F(TrajectoryStitcherTest, StitchesAtElapsedTime)
{
  store_default_trajectory();

  const auto on_grid = stitcher_->compute_planning_origin(at(0.1), make_odom(1.0), uuid_a_, false);
  EXPECT_TRUE(on_grid.stitched);
  EXPECT_NEAR(on_grid.planning_origin.position.x, 1.0, 1e-9);
  EXPECT_NEAR(on_grid.lateral_deviation_m, 0.0, 1e-9);
  EXPECT_NEAR(on_grid.longitudinal_deviation_m, 0.0, 1e-9);

  const auto mid_grid =
    stitcher_->compute_planning_origin(at(0.15), make_odom(1.5), uuid_a_, false);
  EXPECT_TRUE(mid_grid.stitched);
  EXPECT_NEAR(mid_grid.planning_origin.position.x, 1.5, 1e-9);

  const auto before_first_point =
    stitcher_->compute_planning_origin(at(0.05), make_odom(0.5), uuid_a_, false);
  EXPECT_TRUE(before_first_point.stitched);
  EXPECT_NEAR(before_first_point.planning_origin.position.x, 0.5, 1e-9);
}

TEST_F(TrajectoryStitcherTest, TimeOffsetShiftsVirtualPose)
{
  params_.time_offset_s = 0.1;
  stitcher_->update_params(params_);
  store_default_trajectory();

  const auto status = stitcher_->compute_planning_origin(at(0.1), make_odom(2.0), uuid_a_, false);
  EXPECT_TRUE(status.stitched);
...
TEST_F(TrajectoryStitcherTest, HysteresisRearm)
{
  store_default_trajectory();
  arm();
  ...
  const auto still_waiting =
    stitcher_->compute_planning_origin(at(0.1), make_odom(1.0, 0.2), uuid_a_, false);
  EXPECT_FALSE(still_waiting.stitched);
  EXPECT_EQ(still_waiting.reset_reason, "waiting_rearm");

  const auto rearmed =
    stitcher_->compute_planning_origin(at(0.1), make_odom(1.0, 0.05), uuid_a_, false);
  EXPECT_TRUE(rearmed.stitched);
  EXPECT_NEAR(rearmed.planning_origin.position.x, 1.0, 1e-9);
  EXPECT_NEAR(rearmed.planning_origin.position.y, 0.0, 1e-9);
}

TEST_F(TrajectoryStitcherTest, LongitudinalDeviationTriggersReset)
{
  store_default_trajectory();
  arm();
  ...
  EXPECT_FALSE(status.stitched);
  EXPECT_EQ(status.reset_reason, "deviation_exceeded");
  EXPECT_NEAR(status.longitudinal_deviation_m, 3.0, 1e-9);
}
...
TEST_F(TrajectoryStitcherTest, RouteChangeResets)
{
  ...
  stitcher_->set_previous_trajectory(make_trajectory(), make_pose(0.0), uuid_b_);
...
TEST_F(TrajectoryStitcherTest, ElapsedBeyondHorizonResets)
{
  stitcher_->set_previous_trajectory(make_trajectory(3), make_pose(0.0), uuid_a_);

  const auto status = stitcher_->compute_planning_origin(at(0.34), make_odom(3.4), uuid_a_, false);
  EXPECT_FALSE(status.stitched);
  EXPECT_EQ(status.reset_reason, "elapsed_beyond_horizon");
}
...
TEST_F(TrajectoryStitcherTest, NoisyStopTailIsHandled)
{
  ...
  stitcher_->set_previous_trajectory(trajectory, make_pose(0.0), uuid_a_);
...
TEST_F(TrajectoryStitcherTest, DegenerateTrajectoryResets)
{
  ...
  stitcher_->set_previous_trajectory(trajectory, make_pose(5.0), uuid_a_);
