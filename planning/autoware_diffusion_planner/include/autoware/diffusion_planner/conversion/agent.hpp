...
struct AgentData
{
  ...
  // The HDP training converter keeps non-UNKNOWN Polygon tracked objects in
  // neighbor_agents_past.  Legacy waypoint models historically dropped them;
  // keep that behavior as the default for backward compatibility.
  void update_histories(const TrackedObjects & objects, bool include_polygon_objects = false);
