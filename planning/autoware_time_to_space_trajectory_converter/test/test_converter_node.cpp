...
TEST_F(TimeToSpaceNodeTest, HistoryStitchingAndResampling)
{
  auto options = set_params();
  options.parameter_overrides({{"enable_history_stitching", true}});
...

  {
    ...
    while (traj_pub->get_subscription_count() == 0 || odom_pub->get_subscription_count() == 0) {
      if (std::chrono::steady_clock::now() - start_wait > std::chrono::seconds(2)) FAIL();
      rclcpp::spin_some(node);
      std::this_thread::sleep_for(std::chrono::milliseconds(10));
    }
  }
  ...
  for (double x = 0.0; x <= 6.0; x += 0.5) {
    input_odom.pose.pose.position.x = x;
    input_odom.twist.twist.linear.x = 10.0;
    input_odom.header.stamp = node->now();

    input_traj = create_sparse_trajectory();
    for (auto & p : input_traj.points) p.pose.position.x += x;
    input_traj.header.stamp = node->now();

    traj_pub->publish(input_traj);
    odom_pub->publish(input_odom);

    int target_count = callback_count + 1;
    auto step_start = std::chrono::steady_clock::now();
    while (callback_count < target_count) {
      rclcpp::spin_some(node);
      std::this_thread::sleep_for(std::chrono::milliseconds(2));
      if (std::chrono::steady_clock::now() - step_start > std::chrono::seconds(1)) FAIL();
    }
  }

  ASSERT_TRUE(received_msg != nullptr);

  double start_x = received_msg->points.front().pose.position.x;
  EXPECT_NEAR(start_x, 1.0, 0.5);

  int history_count = 0;
  for (const auto & p : received_msg->points) {
    if (rclcpp::Duration(p.time_from_start).seconds() < -0.01) {
      history_count++;
    }
  }

  EXPECT_GE(history_count, 4);
  plot_conversion_result(input_traj, *received_msg, "history_stitching_test.png");
}
