...
void LaneEventClassifierNode::build_classifiers()
{
  ...
  classifiers_.clear();
  if (params_.lane_change.enable_classifier) {
    classifiers_.emplace_back(
      std::make_unique<LaneChangeClassifier>(true, params_.lane_change, lane_tracker_));
  }
  if (params_.lane_crossing.enable_classifier) {
    classifiers_.emplace_back(std::make_unique<IntentionalCrossingClassifier>(true, lane_tracker_));
  }
}
...
bool LaneEventClassifierNode::is_tracking_state_stale()
{
  ...
  if (ego_jumped) {
    tracking_reset_reason_ = "reposition jump (step exceeds reported motion)";
    return true;
  }
  ...
  if (lane_tracker_.is_reference_lane_held()) {
    ...
    if (distance_to_reference && *distance_to_reference > params_.lane_departure_reset_distance_m) {
      tracking_reset_reason_ = "ego departed far from the held reference lane";
      return true;
    }
  }
...
void LaneEventClassifierNode::reset_tracking_state()
{
  ...
  debug_.log_reset(tracking_reset_reason_);
}
  ...
  const autoware_planning_msgs::msg::Trajectory::ConstSharedPtr & trajectory_msg)
{
  ...
  for (auto & classifier : classifiers_) {
    ...
    classifier->fill_debug_markers(input_, lane_tracker_, debug_markers);
  ...
  // (tracked, does not hold the reference lane).
  if (!is_any_event_active && !lane_following_result.is_following) {
    current_state_val = DrivingState::UNKNOWN;
  }
  debug_.publish_markers(debug_markers);

  // Hold the reference lane while any event is active; release it otherwise (arbitration is Phase
  // 8).
