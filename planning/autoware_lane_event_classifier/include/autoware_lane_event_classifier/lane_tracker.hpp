...
#ifndef AUTOWARE_LANE_EVENT_CLASSIFIER__LANE_TRACKER_HPP_
#define AUTOWARE_LANE_EVENT_CLASSIFIER__LANE_TRACKER_HPP_
...
public:
  ...
  /**
   * @brief Replaces the lanelet infrastructure and builds the routing graph (road lanes only).
   * @param lanelet_map_ptr Map to adopt; a null or empty map returns an error.
   */
  ...
  /** @brief Returns the owned lanelet map (for evaluating the gate outside the tracker). */
  [[nodiscard]] const lanelet::LaneletMapPtr & lanelet_map_ptr() const { return lanelet_map_ptr_; }
  /** @brief Returns the owned routing graph. */
...
private:
  ...
  const void * cached_route_ptr_{nullptr};
...
#endif  // AUTOWARE_LANE_EVENT_CLASSIFIER__LANE_TRACKER_HPP_
