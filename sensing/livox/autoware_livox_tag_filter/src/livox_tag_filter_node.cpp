...
void LivoxTagFilterNode::onPointCloud(const sensor_msgs::msg::PointCloud2::ConstSharedPtr msg)
{
  ...
  // convert pointcloud type to xyzirct
  pcl::PointCloud<PointXYZIRCT>::Ptr tag_filtered_points_xyzirct =
    std::make_shared<pcl::PointCloud<PointXYZIRCT>>();

  tag_filtered_points_xyzirct->resize(tag_filtered_points.size());
  for (size_t i = 0; i < tag_filtered_points.size(); ++i) {
    const auto & p = tag_filtered_points[i];
    (*tag_filtered_points_xyzirct)[i].x = p.x;
    (*tag_filtered_points_xyzirct)[i].y = p.y;
    (*tag_filtered_points_xyzirct)[i].z = p.z;
    (*tag_filtered_points_xyzirct)[i].intensity = p.intensity;
    (*tag_filtered_points_xyzirct)[i].return_type = 0;
    (*tag_filtered_points_xyzirct)[i].channel = 0;
    (*tag_filtered_points_xyzirct)[i].time_stamp = 0;
  }

  // Publish ROS message
  auto tag_filtered_msg_ptr = std::make_unique<sensor_msgs::msg::PointCloud2>();
  pcl::toROSMsg(*tag_filtered_points_xyzirct, *tag_filtered_msg_ptr);
