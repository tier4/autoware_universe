...
BlockageDiagComponentBase<NodeT>::BlockageDiagComponentBase(const rclcpp::NodeOptions & options)
: FilterBase<NodeT>("BlockageDiag", options)
{
  {
    // LiDAR configuration
    // Horizontal FoV, expects two values: [min, max]
    angle_range_deg_ = this->template declare_parameter<std::vector<double>>("angle_range");
    // Whether the channel order is top-down (true) or bottom-up (false)
    is_channel_order_top2down_ = this->template declare_parameter<bool>("is_channel_order_top2down");
    ...
    vertical_bins_ = this->template declare_parameter<int>("vertical_bins");
    // The angular resolution of the mask, in degrees.
    horizontal_resolution_ = this->template declare_parameter<double>("horizontal_resolution");

    // Dust detection configuration
    enable_dust_diag_ = this->template declare_parameter<bool>("enable_dust_diag");
    dust_ratio_threshold_ = this->template declare_parameter<float>("dust_ratio_threshold");
    dust_count_threshold_ = this->template declare_parameter<int>("dust_count_threshold");
    dust_kernel_size_ = this->template declare_parameter<int>("dust_kernel_size");
    dust_buffering_frames_ = this->template declare_parameter<int>("dust_buffering_frames");
    dust_buffering_interval_ = this->template declare_parameter<int>("dust_buffering_interval");

    // Blockage detection configuration
    blockage_ratio_threshold_ = this->template declare_parameter<float>("blockage_ratio_threshold");
    blockage_count_threshold_ = this->template declare_parameter<int>("blockage_count_threshold");
    blockage_kernel_ = this->template declare_parameter<int>("blockage_kernel");
    blockage_buffering_frames_ =
      this->template declare_parameter<int>("blockage_buffering_frames");
    blockage_buffering_interval_ =
      this->template declare_parameter<int>("blockage_buffering_interval");

    // Debug configuration
    publish_debug_image_ = this->template declare_parameter<bool>("publish_debug_image");

    // Depth map configuration
    // The maximum distance range of the LiDAR, in meters. The depth map is normalized to this
    // value.
    max_distance_range_ = this->template declare_parameter<double>("max_distance_range");
    ...
    horizontal_ring_id_ = this->template declare_parameter<int>("horizontal_ring_id");
  }
  ...
  if (enable_dust_diag_) {
    ...
    if constexpr (is_rclcpp_node_v<NodeT>) {
      if (publish_debug_image_) {
        single_frame_dust_mask_pub = std::make_unique<image_transport::Publisher>(
          image_transport::create_publisher(this, "blockage_diag/debug/single_frame_dust_mask_image"));
        multi_frame_dust_mask_pub = std::make_unique<image_transport::Publisher>(
          image_transport::create_publisher(this, "blockage_diag/debug/multi_frame_dust_mask_image"));
        blockage_dust_merged_pub = std::make_unique<image_transport::Publisher>(
  ...
  if constexpr (is_rclcpp_node_v<NodeT>) {
    if (publish_debug_image_) {
      lidar_depth_map_pub_ = std::make_unique<image_transport::Publisher>(
        image_transport::create_publisher(this, "blockage_diag/debug/lidar_depth_map"));
      blockage_mask_pub_ = std::make_unique<image_transport::Publisher>(
  ...
  this->set_param_res_ = this->add_on_set_parameters_callback(
    std::bind(&BlockageDiagComponentBase::param_callback, this, _1));
}

template <typename NodeT>
void BlockageDiagComponentBase<NodeT>::run_blockage_check(DiagnosticStatusWrapper & stat) const
{
...
template <typename NodeT>
void BlockageDiagComponentBase<NodeT>::run_dust_check(
...
template <typename NodeT>
cv::Size BlockageDiagComponentBase<NodeT>::get_mask_dimensions() const
{
...
template <typename NodeT>
std::optional<int> BlockageDiagComponentBase<NodeT>::get_horizontal_bin(double azimuth_deg) const
{
...
template <typename NodeT>
std::optional<int> BlockageDiagComponentBase<NodeT>::get_vertical_bin(uint16_t channel) const
{
...
template <typename NodeT>
cv::Mat BlockageDiagComponentBase<NodeT>::make_normalized_depth_image(
  const PCLCloudXYZIRCAEDT & input) const
{
...
template <typename NodeT>
cv::Mat BlockageDiagComponentBase<NodeT>::quantize_to_8u(const cv::Mat & image_16u) const
{
  auto dimensions = get_mask_dimensions();
...
template <typename NodeT>
cv::Mat BlockageDiagComponentBase<NodeT>::make_no_return_mask(const cv::Mat & depth_image) const
{
  auto dimensions = get_mask_dimensions();
...
template <typename NodeT>
cv::Mat BlockageDiagComponentBase<NodeT>::make_blockage_mask(const cv::Mat & no_return_mask) const
{
  auto dimensions = get_mask_dimensions();
...
template <typename NodeT>
cv::Mat BlockageDiagComponentBase<NodeT>::update_time_series_blockage_mask(
  const cv::Mat & blockage_mask)
{
...
template <typename NodeT>
std::pair<cv::Mat, cv::Mat> BlockageDiagComponentBase<NodeT>::segment_into_ground_and_sky(
...
template <typename NodeT>
float BlockageDiagComponentBase<NodeT>::get_nonzero_ratio(const cv::Mat & mask)
{
...
template <typename NodeT>
void BlockageDiagComponentBase<NodeT>::update_ground_blockage_info(
  const cv::Mat & ground_blockage_mask)
{
...
template <typename NodeT>
void BlockageDiagComponentBase<NodeT>::update_sky_blockage_info(const cv::Mat & sky_blockage_mask)
{
...
template <typename NodeT>
void BlockageDiagComponentBase<NodeT>::compute_dust_diagnostics(
  const cv::Mat & no_return_mask, const DebugInfo & debug_info)
{
  ...
  ground_dust_ratio_msg.stamp = this->now();
  if constexpr (is_rclcpp_node_v<NodeT>) {
    ground_dust_ratio_pub_->publish(ground_dust_ratio_msg);
  } else {
    auto loaned_msg = ground_dust_ratio_pub_->borrow_loaned_message();
    *loaned_msg = ground_dust_ratio_msg;
    ground_dust_ratio_pub_->publish(std::move(loaned_msg));
  }
...
template <typename NodeT>
void BlockageDiagComponentBase<NodeT>::publish_debug_info(const DebugInfo & debug_info) const
{
  ...
  ground_blockage_ratio_msg.stamp = this->now();
  if constexpr (is_rclcpp_node_v<NodeT>) {
    ground_blockage_ratio_pub_->publish(ground_blockage_ratio_msg);
  } else {
    auto loaned_msg = ground_blockage_ratio_pub_->borrow_loaned_message();
    *loaned_msg = ground_blockage_ratio_msg;
    ground_blockage_ratio_pub_->publish(std::move(loaned_msg));
  }

  autoware_internal_debug_msgs::msg::Float32Stamped sky_blockage_ratio_msg;
  sky_blockage_ratio_msg.data = sky_blockage_ratio_;
  sky_blockage_ratio_msg.stamp = this->now();
  if constexpr (is_rclcpp_node_v<NodeT>) {
    sky_blockage_ratio_pub_->publish(sky_blockage_ratio_msg);
  } else {
    auto loaned_msg = sky_blockage_ratio_pub_->borrow_loaned_message();
    *loaned_msg = sky_blockage_ratio_msg;
    sky_blockage_ratio_pub_->publish(std::move(loaned_msg));
  }
...
template <typename NodeT>
void BlockageDiagComponentBase<NodeT>::filter(
...
template <typename NodeT>
rcl_interfaces::msg::SetParametersResult BlockageDiagComponentBase<NodeT>::param_callback(
  const std::vector<rclcpp::Parameter> & p)
{
  std::scoped_lock lock(this->mutex_);
  if (get_param(p, "blockage_ratio_threshold", blockage_ratio_threshold_)) {
    RCLCPP_DEBUG(
      this->get_logger(), "Setting new blockage_ratio_threshold to: %f.",
      blockage_ratio_threshold_);
  }
  if (get_param(p, "horizontal_ring_id", horizontal_ring_id_)) {
    RCLCPP_DEBUG(this->get_logger(), "Setting new horizontal_ring_id to: %d.", horizontal_ring_id_);
  }
  if (get_param(p, "vertical_bins", vertical_bins_)) {
    RCLCPP_DEBUG(this->get_logger(), "Setting new vertical_bins to: %d.", vertical_bins_);
  }
  if (get_param(p, "blockage_count_threshold", blockage_count_threshold_)) {
    RCLCPP_DEBUG(
      this->get_logger(), "Setting new blockage_count_threshold to: %d.",
      blockage_count_threshold_);
  }
  if (get_param(p, "is_channel_order_top2down", is_channel_order_top2down_)) {
    RCLCPP_DEBUG(
      this->get_logger(), "Setting new lidar model to: %d. ", is_channel_order_top2down_);
  }
  if (get_param(p, "angle_range", angle_range_deg_)) {
    RCLCPP_DEBUG(
      this->get_logger(), " Setting new angle_range to: [%f , %f].", angle_range_deg_[0],
      angle_range_deg_[1]);
  }
  if (get_param(p, "blockage_buffering_frames", blockage_buffering_frames_)) {
    RCLCPP_DEBUG(
      this->get_logger(), "Setting new blockage_buffering_frames_ to: %d.",
      blockage_buffering_frames_);
  }
  if (get_param(p, "blockage_buffering_interval", blockage_buffering_interval_)) {
    RCLCPP_DEBUG(
      this->get_logger(), "Setting new blockage_buffering_interval_ to: %d.",
      blockage_buffering_interval_);
  }
  if (get_param(p, "dust_kernel_size", dust_kernel_size_)) {
    RCLCPP_DEBUG(this->get_logger(), "Setting new dust_kernel_size_ to: %d.", dust_kernel_size_);
  }
  if (get_param(p, "dust_buffering_frames", dust_buffering_frames_)) {
    RCLCPP_DEBUG(
      this->get_logger(), "Setting new dust_buffering_frames_ to: %d.", dust_buffering_frames_);
  ...
  if (get_param(p, "dust_buffering_interval", dust_buffering_interval_)) {
    RCLCPP_DEBUG(
      this->get_logger(), "Setting new dust_buffering_interval_ to: %d.",
      dust_buffering_interval_);
    dust_buffering_frame_counter_ = 0;
  }
...
template class BlockageDiagComponentBase<rclcpp::Node>;
template class BlockageDiagComponentBase<agnocast::Node>;

}  // namespace autoware::pointcloud_preprocessor

#include <rclcpp_components/register_node_macro.hpp>
RCLCPP_COMPONENTS_REGISTER_NODE(autoware::pointcloud_preprocessor::BlockageDiagComponent)
RCLCPP_COMPONENTS_REGISTER_NODE(autoware::pointcloud_preprocessor::AgnocastBlockageDiagComponent)
