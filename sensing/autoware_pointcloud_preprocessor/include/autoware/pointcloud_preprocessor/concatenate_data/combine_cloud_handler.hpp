...
public:
  ...
  ConcatenatedCloudResult<PointCloud2Traits> combine_pointclouds(
    std::unordered_map<
      std::string, AUTOWARE_MESSAGE_SHARED_PTR(
                     const typename PointCloud2Traits::PointCloudMessage)> & topic_to_cloud_map);
...
protected:
  ...
  static void convert_to_xyzirc_cloud(
    const AUTOWARE_MESSAGE_SHARED_PTR(const typename PointCloud2Traits::PointCloudMessage) &
      input_cloud,
    AUTOWARE_MESSAGE_UNIQUE_PTR(typename PointCloud2Traits::PointCloudMessage) & xyzirc_cloud);

  void correct_pointcloud_motion(
    const AUTOWARE_MESSAGE_UNIQUE_PTR(PointCloud2Traits::PointCloudMessage) & transformed_cloud_ptr,
    const std::vector<rclcpp::Time> & pc_stamps,
    std::unordered_map<rclcpp::Time, Eigen::Matrix4f, RclcppTimeHash> & transform_memo,
    AUTOWARE_MESSAGE_UNIQUE_PTR(PointCloud2Traits::PointCloudMessage) &
