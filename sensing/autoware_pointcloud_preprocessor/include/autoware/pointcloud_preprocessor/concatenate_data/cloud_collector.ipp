...
void CloudCollector<MsgTraits>::process_pointcloud(
  ...
  const AUTOWARE_MESSAGE_SHARED_PTR(const typename MsgTraits::PointCloudMessage) cloud)
{
  std::lock_guard<std::mutex> concatenate_lock(concatenate_mutex_);
...
template <typename MsgTraits>
CollectorStatus CloudCollector<MsgTraits>::get_status()
{
  return status_;
}
...
ConcatenatedCloudResult<MsgTraits> CloudCollector<MsgTraits>::concatenate_pointclouds(
  std::unordered_map<
    std::string, AUTOWARE_MESSAGE_SHARED_PTR(const typename MsgTraits::PointCloudMessage)>
    topic_to_cloud_map)
{
