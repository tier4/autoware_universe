^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Changelog for package autoware_imu_corrector
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

0.38.0 (2024-11-08)
-------------------
* unify package.xml version to 0.37.0
* chore(autoware_imu_corrector): refactored the imu corrector into the autoware namespace (`#8222 <https://github.com/autowarefoundation/autoware.universe/issues/8222>`_)
  * chore: refactored the imu corrector into the autoware namespace
  * chore: reverted to non-exported includes
  ---------
  Co-authored-by: Yamato Ando <yamato.ando@gmail.com>
* Contributors: Kenzo Lobos Tsunekawa, Yutaka Kondo

Forthcoming
-----------
* fix(cpplint): include what you use - sensing (`#9571 <https://github.com/tier4/autoware.universe/issues/9571>`_)
* fix: fix ticket links to point to https://github.com/autowarefoundation/autoware.universe (`#9304 <https://github.com/tier4/autoware.universe/issues/9304>`_)
* chore(package.xml): bump version to 0.38.0 (`#9266 <https://github.com/tier4/autoware.universe/issues/9266>`_) (`#9284 <https://github.com/tier4/autoware.universe/issues/9284>`_)
  * unify package.xml version to 0.37.0
  * remove system_monitor/CHANGELOG.rst
  * add changelog
  * 0.38.0
  ---------
* chore(autoware_imu_corrector): refactored the imu corrector into the autoware namespace (`#8222 <https://github.com/tier4/autoware.universe/issues/8222>`_)
  * chore: refactored the imu corrector into the autoware namespace
  * chore: reverted to non-exported includes
  ---------
  Co-authored-by: Yamato Ando <yamato.ando@gmail.com>
* Contributors: Esteve Fernandez, Kenzo Lobos Tsunekawa, M. Fatih Cırıt, Yutaka Kondo

0.26.0 (2024-04-03)
-------------------
