...
import config as CF
from csv_acc_cmd_reader import CSVReader
import numpy as np
...
class ViewPlot(Node):
    def __init__(self, args):
        ...
        acc_diff_thr = args.acc_diff_thr
        ...
        acc_cmd_data = self.cr.getAccCmdData()
    ...
    def view_pedal_accel_graph(
        ...
        # plot all data
        pedal_list = [0 for i in range(len(CF.ACC_CMD_LIST))]
        if velocity_map_list[vel_list_idx] is not None:
            ...
            for pedal in velocity_map_list[vel_list_idx][:, 1]:
                min_pedal = 10
                for pedal_idx, ref_pedal in enumerate(CF.ACC_CMD_LIST):
        ...
        # plot average data
        plotter.scatter(CF.ACC_CMD_LIST, average_map[:, vel_list_idx], "red", label="average")

        # add label of standard deviation
        plotter.scatter([], [], "black", label="std dev")

        # plot average text
        for i in range(len(CF.ACC_CMD_LIST)):
        ...
        pedal_lim = [CF.ACC_CMD_LIST[0] - 0.05, CF.ACC_CMD_LIST[-1] + 0.05]
        accel_lim = [-5.0, 5.0]
    ...
    def load_map(self, csv_dir):
        try:
            accel_pedal_list = []
            accel_acc_list = []
            with open(csv_dir + "acceleration_map.csv") as f:
            ...
            return np.array(accel_pedal_list).T
        except OSError as e:
            print(e)
            return [], []
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-l",
        "--log-file",
        default=package_path + "/config/acc_cmd_test_log.csv",
        type=str,
    ...
    parser.add_argument(
        "--acc-diff-thr", default=0.03, type=float, help="valid pedal diff threshold"
    )
    parser.add_argument(
