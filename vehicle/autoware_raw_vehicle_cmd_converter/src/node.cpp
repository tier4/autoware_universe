...
: Node("autoware_raw_vehicle_cmd_converter_node", node_options)
{
  ...
  pub_meas_to_actuation_cmd_ = create_publisher<Float64Stamped>("~/output/meas_to_actuation_cmd", 1);
...
void RawVehicleCommandConverterNode::publishActuationCmd()
{
  ...
  // Calculate and publish the time difference
  ...
  pub_meas_to_actuation_cmd_->publish(*timestamp_diff);
}
