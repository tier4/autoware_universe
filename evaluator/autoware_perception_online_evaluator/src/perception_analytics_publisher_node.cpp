  ...
  parameters_(std::make_shared<AnalyticsParameters>())
{
  ...
  meas_to_tracked_latency_sub_ = create_subscription<Float64Stamped>(
    ...
    [this](const agnocast::ipc_shared_ptr<const Float64Stamped> & msg) {
  ...
  tf_buffer_ = std::make_shared<agnocast::Buffer>(this->get_clock());
  transform_listener_ = std::make_shared<agnocast::TransformListener>(*tf_buffer_, *this);
}
...
void PerceptionAnalyticsPublisherNode::publishPerceptionAnalytics()
{
  ...
  auto metrics_msg = perception_analytics_pub_->borrow_loaned_message();
