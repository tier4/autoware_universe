...
| 名称                               | 型      | 説明                                                                                                                                                                                                                                                                                                                                                          | デフォルト値 |
| :--------------------------------- | :------ | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | :----------- |
| `transition_timeout`               | `float` | ステート遷移が **指定時間内に完了しない**場合、遷移が失敗と見なされます。                                                                                                                                                                                                                                                                                     | 10.0         |
| `frequency_hz`                     | `float` | 実行 hz                                                                                                                                                                                                                                                                                                                                                       | 10.0         |
| `enable_engage_on_driving`         | `bool`  | 車両 **走行中に自動運転モードをオンにする**かどうかを指定します。 false の場合、車両速度が 0 以外の状況では走行モードへの切り替えを拒否します。パラメータを調整せずにこの機能を使用すると、急減速などの問題が発生する可能性があることに注意してください。使用する前に、走行条件と vehicle_cmd_gate 遷移フィルタが適切に調整されていることを確認してください。 | 0.1          |
