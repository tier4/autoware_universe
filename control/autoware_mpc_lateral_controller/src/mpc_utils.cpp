  ...
  trajectory_follower::LateralHorizon & horizon, std::string & reason)
{
  ...
  reason.clear();
  if (
    !std::isfinite(age_s) || !std::isfinite(timeout_s) || timeout_s <= 0.0 || age_s < 0.0 ||
    age_s > timeout_s) {
    reason = "passthrough trajectory is stale or future-dated";
    return false;
  }
  if (trajectory.points.size() < 2) {
    reason = "passthrough requires at least two timed commands";
    return false;
  }
  const double first_time = rclcpp::Duration(trajectory.points.front().time_from_start).seconds();
  const double dt = rclcpp::Duration(trajectory.points[1].time_from_start).seconds() - first_time;
  if (first_time < 0.0 || !std::isfinite(dt) || dt <= 0.0) {
    reason = "invalid passthrough command interval";
    return false;
  }
  for (size_t i = 0; i < trajectory.points.size(); ++i) {
    const auto & p = trajectory.points[i];
    const double time = rclcpp::Duration(p.time_from_start).seconds();
    if (
      !std::isfinite(p.front_wheel_angle_rad) || !std::isfinite(p.longitudinal_velocity_mps) ||
      !std::isfinite(p.pose.position.x) || !std::isfinite(p.pose.position.y) ||
      !std::isfinite(p.pose.position.z) || !std::isfinite(p.pose.orientation.x) ||
      !std::isfinite(p.pose.orientation.y) || !std::isfinite(p.pose.orientation.z) ||
      !std::isfinite(p.pose.orientation.w) ||
      (i > 0 && time <= rclcpp::Duration(trajectory.points[i - 1].time_from_start).seconds()) ||
      std::abs(time - (first_time + static_cast<double>(i) * dt)) > 1.0e-6) {
      reason = "passthrough requires finite commands and uniformly increasing timestamps";
      return false;
    }
  }
  if (age_s > dt * static_cast<double>(trajectory.points.size())) {
    reason = "passthrough command horizon has expired";
    return false;
  }
  // LateralHorizon has one fixed interval. Reject irregular input instead of silently
  // resampling its commands. The first command is issued now, regardless of x[1]'s timestamp.
  horizon.time_step_ms = dt * 1000.0;
  horizon.controls.reserve(trajectory.points.size());
  for (const auto & point : trajectory.points) {
    autoware_control_msgs::msg::Lateral command;
    command.steering_tire_angle = point.front_wheel_angle_rad;
    horizon.controls.push_back(command);
  }
  return true;
}
...
{
  if (
    !std::isfinite(previous_angle) || !std::isfinite(offset) || !std::isfinite(angle_limit) ||
    angle_limit <= 0.0 || !std::isfinite(rate_limit) || rate_limit <= 0.0 ||
    !std::isfinite(command_dt) || command_dt <= 0.0 ||
    (horizon.controls.size() > 1 &&
     (!std::isfinite(horizon.time_step_ms) || horizon.time_step_ms <= 0.0))) {
    throw std::invalid_argument("invalid steering passthrough output limits or timing");
  }
  double previous = previous_angle;
  for (size_t i = 0; i < horizon.controls.size(); ++i) {
    auto & command = horizon.controls[i];
    // Bound in both model and published calibration frames.
    const double model_angle =
      std::clamp(static_cast<double>(command.steering_tire_angle), -angle_limit, angle_limit);
    command.steering_tire_angle =
      static_cast<float>(std::clamp(model_angle - offset, -angle_limit, angle_limit));
    const double dt = i == 0 ? command_dt : horizon.time_step_ms * 0.001;
    command.steering_tire_rotation_rate = static_cast<float>(std::clamp(
      (static_cast<double>(command.steering_tire_angle) - previous) / dt, -rate_limit, rate_limit));
    previous = command.steering_tire_angle;
  }
}
