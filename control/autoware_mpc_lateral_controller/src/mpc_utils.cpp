...
Trajectory convertToAutowareTrajectory(const MPCTrajectory & input, const double wheelbase)
{
  ...
  for (size_t i = 0; i < input.size(); ++i) {
    ...
    // A non-zero wheelbase is supplied for internally predicted trajectories, whose steering
    // field is not populated by the vehicle-model helpers. Preserve their existing geometric
    // conversion; otherwise carry the input trajectory steering state through debug conversions.
    if (wheelbase != 0.0) {
