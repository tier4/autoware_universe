...
void SimplePlanningSimulator::initialize_vehicle_model(const std::string & vehicle_model_type_str)
{
  ...
  // Understeer coefficient for the augmented-bicycle yaw rate
  // ω = vx · tan(δ) / (L + k_us · vx²). Default 0 keeps the ideal kinematic
  // bicycle model untouched. Only consumed by DELAY_STEER_ACC_GEARED_WO_FALL_GUARD.
