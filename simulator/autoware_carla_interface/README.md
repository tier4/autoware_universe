...
| `map_origin_x`                    | double | 0.0                                                                               | X offset from the CARLA world origin to the Autoware map frame origin. Required whenever the CARLA level's local coordinates don't already coincide with the lanelet2/PCD map frame (e.g. a custom map converted from a real-world-georeferenced lanelet2 map). Leave at 0.0 for stock CARLA towns. See [Aligning a georeferenced/custom map](#aligning-a-georeferencedcustom-map).                |
| `map_origin_y`                    | double | 0.0                                                                               | Same as `map_origin_x`, for the Y axis.                                                                                                                                                                              |
...
#### Aligning a georeferenced/custom map

The bridge publishes the ego's raw CARLA world position as the GNSS pose in the `map` frame (`carla_ros2_interface.pose()`), and converts RViz-set initial poses back to a CARLA transform (`initialpose_callback`). This 1:1 mapping only works when CARLA's world origin coincides with the Autoware map frame origin, which is true for the stock "y-axis inverted" CARLA towns (their lanelet2/PCD maps are derived directly from CARLA's own OpenDRIVE, with no translation).

For a custom map converted from a real-world-georeferenced lanelet2 map (e.g. one exported with an MGRS/UTM projector), the CARLA level typically gets re-centered around a local origin during the lanelet2 -> OpenDRIVE conversion, so CARLA world coordinates are offset from the map frame's `local_x`/`local_y`. Set `map_origin_x`/`map_origin_y` to that offset so `map_local = carla_xy + map_origin`:

1. Find the `.xodr` used to build the CARLA level and read its `geoReference` tag, e.g. `+proj=utm +zone=54 +lat_0=<lat0> +lon_0=<lon0> ...`.
2. Project `(lat0, lon0)` with a standard UTM projection for that zone (not the custom `lat_0`/`lon_0`, which `+proj=utm` ignores) to get `(utm_x, utm_y)`.
3. Subtract the map's MGRS 100 km grid-square offset (e.g. `300000`/`3900000` for grid `54SUE`) from `(utm_x, utm_y)` to get `(map_origin_x, map_origin_y)` in the same local frame as the map's `local_x`/`local_y` tags.

Sanity-check by comparing the bounding box of `local_x`/`local_y` across the lanelet2 map against the bounding box of `x`/`y` in the `.xodr`'s `<geometry>` elements shifted by `map_origin` — they should match closely.
