...
- the trajectory tensor [1, N, 3] supplies the horizon: N waypoints over 4 s, so
  the model plans them 4/N apart. The trajectory the node publishes is always on
  a 0.1 s grid; a model that plans coarser (the 2 Hz checkpoint emits 8
  waypoints 0.5 s apart) is interpolated onto that grid by the postprocessor,
  the way LatentDrive-TRT's deploy_utils.plan_to_poses does, so nothing has to
  be configured for it here.
...
def main() -> int:
    ...
    plan_step = HORIZON_SECONDS / num_waypoints
    if plan_step < SUPPORTED_TIME_STEP - 1e-9:
        raise SystemExit(
            f"{planner_path.name} emits {num_waypoints} waypoints over {HORIZON_SECONDS} s "
            f"({plan_step:.3g} s apart), finer than the {SUPPORTED_TIME_STEP} s trajectory step; "
            "the node can interpolate a coarser plan onto that grid but never subsample a finer "
            "one")
    ...
    lines = [
        ...
        f"      horizon_seconds: {HORIZON_SECONDS}  # {num_waypoints} waypoints"
        + ("" if abs(plan_step - SUPPORTED_TIME_STEP) < 1e-9
           else f" {plan_step:.3g} s apart, interpolated onto the {SUPPORTED_TIME_STEP} s grid"),
    ...
    print(f"  frames={num_frames} interval={args.frame_interval}s input={width}x{height} "
          ...
          + ("" if abs(plan_step - SUPPORTED_TIME_STEP) < 1e-9
             else f" ({plan_step:.3g}s apart, interpolated to {SUPPORTED_TIME_STEP}s)"))
    return 0
