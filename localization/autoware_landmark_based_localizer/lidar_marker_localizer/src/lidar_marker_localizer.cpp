  ...
  const PointCloud2::ConstSharedPtr & points_msg_ptr)
{
  ...
  if (param_.marker_to_vehicle_offset_y >= 0) {
    center_intensity_grid_msg.info.origin.position.y += center_intensity_grid_msg.info.height * center_intensity_grid_msg.info.resolution;
    center_intensity_grid_msg.info.origin.orientation = autoware::universe_utils::createQuaternionFromRPY(M_PI, 0.0, 0.0);
  }
  else {
    center_intensity_grid_msg.info.origin.orientation = autoware::universe_utils::createQuaternionFromRPY(0.0, 0.0, 0.0);
  }
