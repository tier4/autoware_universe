...
{
  RCLCPP_INFO(
    this->get_logger(), "points_callback called. header.stamp=%u",
    points_msg_ptr->header.stamp.sec);
...
{
  RCLCPP_INFO(this->get_logger(), "main_process called.");
  ...
  if (detected_landmarks.empty()) {
    RCLCPP_WARN(this->get_logger(), "No marker detected: marker_pointcloud will not be published.");
  } else {
    RCLCPP_INFO(
      this->get_logger(),
      "Publishing marker_pointcloud (detected marker count: %zu, nearest marker: [x=%.2f, y=%.2f, "
      "z=%.2f])",
      detected_landmarks.size(), nearest_detected_landmark.pose.position.x,
      nearest_detected_landmark.pose.position.y, nearest_detected_landmark.pose.position.z);
  }
