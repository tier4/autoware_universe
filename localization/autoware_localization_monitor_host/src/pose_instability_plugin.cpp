...
#include "autoware/localization_monitor_host/pose_instability_plugin.hpp"
...
void PoseInstabilityPlugin::initialize(const std::string & name, rclcpp::Node * node_ptr)
{
  LocalizationMonitorPluginBase::initialize(name, node_ptr);

  diff_pose_pub_ =
    node_ptr->create_publisher<PoseStamped>("~/debug/diff_pose", 10);
  diagnostics_pub_ =
    node_ptr->create_publisher<DiagnosticArray>("/diagnostics", 10);
}

void PoseInstabilityPlugin::set_up_params()
{
  auto * node = get_node_ptr();

  timer_period_ = node->declare_parameter<double>("pose_instability.timer_period");
  heading_velocity_maximum_ =
    node->declare_parameter<double>("pose_instability.heading_velocity_maximum");
  heading_velocity_scale_factor_tolerance_ =
    node->declare_parameter<double>("pose_instability.heading_velocity_scale_factor_tolerance");
  angular_velocity_maximum_ =
    node->declare_parameter<double>("pose_instability.angular_velocity_maximum");
  angular_velocity_scale_factor_tolerance_ =
    node->declare_parameter<double>("pose_instability.angular_velocity_scale_factor_tolerance");
  angular_velocity_bias_tolerance_ =
    node->declare_parameter<double>("pose_instability.angular_velocity_bias_tolerance");
  pose_estimator_longitudinal_tolerance_ =
    node->declare_parameter<double>("pose_instability.pose_estimator_longitudinal_tolerance");
  pose_estimator_lateral_tolerance_ =
    node->declare_parameter<double>("pose_instability.pose_estimator_lateral_tolerance");
  pose_estimator_vertical_tolerance_ =
    node->declare_parameter<double>("pose_instability.pose_estimator_vertical_tolerance");
  pose_estimator_angular_tolerance_ =
    node->declare_parameter<double>("pose_instability.pose_estimator_angular_tolerance");

  enable_validation_flags_ = {
    node->declare_parameter<bool>("pose_instability.enable_validation.position_x"),
    node->declare_parameter<bool>("pose_instability.enable_validation.position_y"),
    node->declare_parameter<bool>("pose_instability.enable_validation.position_z"),
    node->declare_parameter<bool>("pose_instability.enable_validation.angle_x"),
    node->declare_parameter<bool>("pose_instability.enable_validation.angle_y"),
    node->declare_parameter<bool>("pose_instability.enable_validation.angle_z"),
  };
}

void PoseInstabilityPlugin::on_parameter(const std::vector<rclcpp::Parameter> & parameters)
{
  for (const auto & param : parameters) {
    const auto & name = param.get_name();
    if (name == "pose_instability.timer_period") {
      timer_period_ = param.as_double();
    } else if (name == "pose_instability.heading_velocity_maximum") {
      heading_velocity_maximum_ = param.as_double();
    } else if (name == "pose_instability.heading_velocity_scale_factor_tolerance") {
      heading_velocity_scale_factor_tolerance_ = param.as_double();
    } else if (name == "pose_instability.angular_velocity_maximum") {
      angular_velocity_maximum_ = param.as_double();
    } else if (name == "pose_instability.angular_velocity_scale_factor_tolerance") {
      angular_velocity_scale_factor_tolerance_ = param.as_double();
    } else if (name == "pose_instability.angular_velocity_bias_tolerance") {
      angular_velocity_bias_tolerance_ = param.as_double();
    } else if (name == "pose_instability.pose_estimator_longitudinal_tolerance") {
      pose_estimator_longitudinal_tolerance_ = param.as_double();
    } else if (name == "pose_instability.pose_estimator_lateral_tolerance") {
      pose_estimator_lateral_tolerance_ = param.as_double();
    } else if (name == "pose_instability.pose_estimator_vertical_tolerance") {
      pose_estimator_vertical_tolerance_ = param.as_double();
    } else if (name == "pose_instability.pose_estimator_angular_tolerance") {
      pose_estimator_angular_tolerance_ = param.as_double();
    } else if (name == "pose_instability.enable_validation.position_x") {
      enable_validation_flags_[0] = param.as_bool();
    } else if (name == "pose_instability.enable_validation.position_y") {
      enable_validation_flags_[1] = param.as_bool();
    } else if (name == "pose_instability.enable_validation.position_z") {
      enable_validation_flags_[2] = param.as_bool();
    } else if (name == "pose_instability.enable_validation.angle_x") {
      enable_validation_flags_[3] = param.as_bool();
    } else if (name == "pose_instability.enable_validation.angle_y") {
      enable_validation_flags_[4] = param.as_bool();
    } else if (name == "pose_instability.enable_validation.angle_z") {
      enable_validation_flags_[5] = param.as_bool();
    }
  }
}

void PoseInstabilityPlugin::evaluate(
  const MonitorData & data, diagnostic_updater::Updater & /*updater*/)
{
  ...
  if (first_run_) {
    prev_odom_ = data.latest_odom;
    first_run_ = false;
    return;
  }

  if (data.twist_buffer.empty() || !prev_odom_.has_value()) {
    return;
  }

  const auto & latest_odom = data.latest_odom.value();
  const auto & prev_odom = prev_odom_.value();
  const rclcpp::Time latest_odometry_time(latest_odom.header.stamp);
  const rclcpp::Time prev_odometry_time(prev_odom.header.stamp);

  auto twist_buffer = data.twist_buffer;

  while (twist_buffer.size() > 1) {
    if (rclcpp::Time(twist_buffer[1].header.stamp) < prev_odometry_time) {
  ...
  dead_reckon(prev_pose, latest_odometry_time, twist_buffer, dr_pose);

  const Pose latest_ekf_pose = latest_odom.pose.pose;
  const Pose ekf_to_dr =
    autoware_utils_geometry::inverse_transform_pose(*dr_pose, latest_ekf_pose);
  ...
  status.hardware_id = "pose_instability_detector";
  bool all_ok = true;
...
PoseInstabilityPlugin::ThresholdValues PoseInstabilityPlugin::calculate_threshold(
  double interval_sec) const
{
  ...
  for (int i = 0; i < 4; i++) {
    ...
    const double lateral_difference_candidate = std::abs(
...
void PoseInstabilityPlugin::dead_reckon(
...
std::deque<PoseInstabilityPlugin::TwistWithCovarianceStamped>
PoseInstabilityPlugin::clip_out_necessary_twist(
